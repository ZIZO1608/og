/* ==========================================================================
   OG SYSTEM — application shell  ·  2/17: LANGUAGE (I18N base table + t())
   --------------------------------------------------------------------------
   Split from the original js/app.js (lines 41-904). Loads after
   app-state.js. This is the base translation table — a second wave of keys
   (Yalla Wear, labels, bulk-select, the delivery office, money) is merged in
   later by js/app-i18n-extra.js, which must load after this file.
   ========================================================================== */

/* ------------------------------------------------------------ 2. LANGUAGE */

var I18N = {
  en: {
    nav_main: 'Operations', nav_ops: 'Business',
    nav_dashboard: 'Dashboard', nav_pos: 'Cashier', nav_products: 'Products',
    nav_warehouse: 'Warehouse', nav_customers: 'Customers', nav_print: 'Print Jobs',
    nav_reports: 'Reports', nav_settings: 'Settings',
    tagline: 'Sneakers & Streetwear', live: 'OG System', search_top: 'Search the shop…', search_products: 'Search products…', search_customers: 'Search customers…',

    admin: 'Admin',
    notifications: 'Notifications', view_all: 'View all', no_results: 'Nothing matched that search',
    nt_all_read: 'Every alert is marked as read',

    /* --- account --- */
    sign_out: 'Sign out', signing_out: 'Signing out…',
    change_pw: 'Change password', my_account: 'My account',
    pw_current: 'Current password', pw_new: 'New password', pw_again: 'Type it again',
    pw_changed: 'Password changed', pw_reauth: 'Sign in again with your new password',
    pw_mismatch: 'The two new passwords do not match',
    pw_must_change: 'Your password was reset — please change it',
    role_delivery: 'Delivery', role_partner: 'Print partner',
    /* --- roles grid --- */
    roles_editable: 'Tick what each role may do. Saved straight away.',
    perm_saved: 'Role updated', perm_refused: 'Some changes were refused',
    pg_till: 'Cashier', pg_stock: 'Stock', pg_products: 'Products',
    pg_customers: 'Customers', pg_money: 'Money', pg_print: 'Printing',
    pg_admin: 'Management', pg_partner: 'Yalla Wear', pg_delivery: 'Deliveries',

    /* --- the role home screens --- */
    hi_morning: 'Good morning', hi_afternoon: 'Good afternoon', hi_evening: 'Good evening',
    my_sales_today: 'My sales today', my_invoices: 'My invoices',
    my_last_sales: 'My last sales', open_till: 'Open the Cashier',
    nothing_sold_yet: 'Nothing rung up yet today', first_sale_hint: 'Scan something to start',
    low_on_shelf: 'Running out on the shelf', low_on_shelf_sub: 'ask the back for more',
    shelf_all_good: 'The shelf is well stocked',
    back_title: 'The back room', back_sub: 'What came in, what needs moving',
    arrived_today: 'Arrived today', to_move_out: 'Waiting to go to the floor',
    empty_on_floor: 'Empty sizes on the floor', open_orders: 'Orders on the way',
    back_receive: 'Book something in', back_count: 'Start a count',
    back_nothing: 'Nothing waiting', back_nothing_sub: 'the floor is stocked from the back',

    /* --- deliveries --- */
    nav_deliveries: 'Deliveries',
    dl_title: 'Deliveries', dl_sub: 'What is going out and what came back',
    dl_my_runs: 'My runs today', dl_my_runs_sub: 'tap a card when it is done',
    dl_waiting: 'To take out', dl_out: 'On the road', dl_delivered: 'Delivered',
    dl_failed: 'Not delivered', dl_to_collect: 'Collect', dl_collected: 'Collected',
    dl_nothing_owed: 'Already paid', dl_call: 'Call', dl_map: 'Map',
    dl_take: 'Take it out', dl_done: 'Delivered', dl_fail: 'Could not deliver',
    dl_why: 'What happened?', dl_why_ph: 'nobody home, wrong address, refused…',
    dl_none: 'Nothing to take out', dl_none_sub: 'you are all caught up',
    dl_none_board: 'No deliveries yet',
    dl_none_board_sub: 'send one out from a sale on the Cashier or Reports screen',
    dl_unassigned: 'Nobody yet', dl_assign: 'Send out for delivery',
    dl_address: 'Address', dl_address_ph: 'street, building, flat — enough to find the door',
    dl_phone: 'Phone',
    dl_marked: 'Marked', dl_sent: 'Out for delivery',
    dl_no_address: 'Type an address first', dl_pick_driver: 'Pick a driver',
    retry: 'Try again', loading: 'Loading…',

    /* --- the printed receipt --- */
    rc_title: 'Receipt', rc_full_page: 'Full page invoice',
    rc_policy: 'Exchange within 48 hours with this receipt',
    rc_paper: 'Receipt paper', rc_80: '80 mm', rc_58: '58 mm',
    rc_paper_hint: 'The roll your printer takes. 80 mm is standard.',

    /* --- the 80mm thermal receipt (js/receipt.js) — drawn on canvas, so
       these are read directly off I18N.en / I18N.ar rather than through
       t(), because the printed slip needs both languages on the page at
       once, never just whichever one the app happens to be showing. */
    rc2_invoice: 'Invoice', rc2_datetime: 'Date & time', rc2_cashier: 'Cashier',
    rc2_customer: 'Customer', rc2_phone: 'Phone', rc2_points_balance: 'Points balance',
    rc2_size: 'Size', rc2_line_discount: 'Discount',
    rc2_subtotal: 'Subtotal', rc2_discount: 'Discount', rc2_points_used: 'Points used',
    rc2_total: 'TOTAL', rc2_payment: 'Payment', rc2_to_collect: 'Amount to collect (COD)',
    rc2_points_earned: 'Points earned', rc2_shop_copy: 'SHOP COPY',
    rc2_gift_copy: 'GIFT RECEIPT', rc2_exchange_before: 'Exchange before',
    gift_receipt: 'Gift receipt',
    gift_pick_title: 'Gift receipt',
    gift_pick_hint: 'Tick what goes in the gift bag. The slip shows no prices — only what it is, when it was bought, and the date it can be exchanged by.',
    gift_pick_none: 'Tick at least one item.',
    pos_gift: 'This sale has a gift in it',
    rc2_scan_exchange: 'Scan for exchange',
    rc_approve_title: 'Approve receipt',
    rc_approve_hint: 'This is exactly what will print. Check it, then print.',
    rc_approve_cancel: 'Not yet',
    print_receipt: 'Print receipt', printing: 'Printing',
    print_sent: 'Sent to the printer', print_retry: 'Retry from the receipt.',

    rc3_title: 'Receipt printer', rc3_sub: 'The 80mm slip a customer walks out holding',
    rc3_transport: 'Connection', rc3_transport_network: 'Network', rc3_transport_usb: 'USB (this computer)',
    rc3_printer_share: 'Printer share', rc3_printer_share_hint: 'The Windows printer share this was set up under — see the setup steps for adding it as Generic / Text Only and sharing it.',
    rc3_instagram: 'Instagram URL', rc3_telegram: 'Telegram URL', rc3_maps_url: 'Maps link',
    rc3_host: 'Printer address (LAN IP)', rc3_port: 'Port', rc3_branch: 'Branch name',
    rc3_auto_print: 'Auto-print on sale', rc3_auto_print_hint: 'Off for a client demo — a printer chattering through a meeting is a bad look.',
    rc3_confirm_print: 'Approve before printing',
    rc3_confirm_print_hint: 'Show the finished receipt and print it only when someone clicks Print. Off sends it straight to the printer — faster at a busy counter, but nobody sees the slip before the customer does.',
    rc3_copies: 'Copies', rc3_cut_mode: 'Cut', rc3_cut_partial: 'Partial', rc3_cut_full: 'Full',
    rc3_ink: 'Print darkness',
    rc3_ink_normal: 'Normal', rc3_ink_dark: 'Dark', rc3_ink_darker: 'Darker',
    rc3_ink_hint: 'How hard the printer burns each dot. Turn this up if the small lines come out faint or patchy, down if the letters fill in and go solid black.',
    rc3_gift_window: 'Gift exchange window (days)',
    rc3_gift_window_hint: 'Longer than the normal window on purpose — a present is bought before it is given. The gift slip prints the last day it can be exchanged.',
    rc3_gift_policy_ar: 'Gift slip wording (Arabic)',
    rc3_gift_policy_en: 'Gift slip wording (English)',
    rc3_show_barcode: 'Barcode', rc3_show_loyalty: 'Loyalty lines',
    rc3_show_barcode_hint: 'The invoice number as bars, so a return can be scanned instead of typed.',
    rc3_show_loyalty_hint: 'Points earned and the balance, printed under the total for customers on the scheme.',
    rc3_footer_ar: 'Footer (Arabic)', rc3_footer_en: 'Footer (English)',
    rc3_policy_ar: 'Return policy (Arabic)', rc3_policy_en: 'Return policy (English)',
    rc3_save: 'Save receipt settings', rc3_saved: 'Receipt settings saved',

    /* The five subjects this card holds, as headings over their own group. */
    rc3_g_printer: 'Which printer',
    rc3_g_head: 'What prints at the top',
    rc3_g_head_note: 'The header of every slip. The street address is not printed — the customer is standing in the shop.',
    rc3_g_how: 'How it prints',
    rc3_g_how_note: 'When the slip comes out, how many, and how hard the head burns it.',
    rc3_g_slip: 'What appears on the slip',
    rc3_g_slip_note: 'Both blocks print bilingual: the Arabic above the English, in the order the paper reads.',
    rc3_g_gift: 'The gift slip',

    /* --- thermal labels (XP-235B) — the label preview, the station and the
       shelf labels (rc_paper/hw_printer strings above are the receipt's) --- */
    lbl_title: 'Thermal labels',
    lbl_print_now: 'Print now', lbl_preview_title: 'Label preview',
    lbl_queued: 'Queued {n} labels for {station}',
    lb_sizes: '{n} sizes', lb_sel: '{n} ticked', lb_print_all: 'Print all sizes',
    lbl_pick_station: 'Pick a station first',
    lbl_calibrate_sent: 'Calibration sent',
    lbl_fallback: 'Falls back to Code128 — too narrow for EAN-13',
    lbl_station: 'Station', lbl_preset: 'Size', lbl_qty: 'Qty',
    lbl_batch_total: 'labels total',
    lbl_thermal_section: 'Thermal labels (XP-235B)',
    lbl_thermal_sub: 'The barcode sticker on the box, printed over USB',
    lbl_queue_title: 'Print queue', lbl_queue_empty: 'Nothing queued',
    lbl_cancel: 'Cancel',
    lbl_unknown_code: 'That code doesn’t match any product',
    lbl_attach_code: 'Attach this code to a product',
    lbl_attach_search: 'Search products…', lbl_attach_save: 'Save',
    lbl_save: 'Save label settings', lbl_saved: 'Label settings saved',
    lbl_host: 'Printer address (LAN, only for the TCP transport)',
    lbl_transport: 'Transport', lbl_transport_agent: 'Agent (USB laptop)',
    lbl_transport_tcp: 'Direct (LAN)', lbl_density: 'Density',
    lbl_gap: 'Gap between labels (mm)',

    /* --- 60 x 40 mm labels (js/labels60.js) — the roll actually loaded --- */
    l60_shelf_title: 'Shelf labels',
    l60_room: 'Room', l60_from: 'From', l60_to: 'To', l60_all: 'All',
    l60_print: 'Print {n}', l60_true_size: '{n} label(s), shown at real size — 60 x 40 mm',
    l60_per_pair: 'One label per pair in stock',
    l60_no_shelves: 'No shelves have been laid out in this warehouse yet.',
    l60_wh_unnumbered: 'This warehouse has no label number, so a shelf barcode cannot be built.',
    l60_no_wh_code: 'no warehouse number', l60_no_barcode: 'code cannot be encoded',
    l60_not_logged: 'Printed, but not written to the label log',

    /* --- the warehouse map (js/shelfmap.js) --- */
    sm_room: 'Room',
    sm_edit: 'Edit layout', sm_done: 'Done',
    sm_no_rooms: 'No rooms have been laid out yet.',
    sm_reset_view: 'Reset view',
    sm_room_loading: 'Loading the room…',
    sm_room_hint: 'Drag to look around · scroll to zoom · click a shelf',
    sm_room_edit_hint: 'Drag a rack onto a wall to move it · pull a wall to resize the room — racks that no longer fit are narrowed',
    sm_drag_span_cm: '{a}–{b} cm',
    sm_drag_slid: 'slid along to fit',
    sm_drag_corner: 'moved out of the corner',
    sm_drag_floor: 'On the floor, on no wall',
    sm_drag_short_cm: 'That wall has {n} cm and the rack is {w} cm wide',
    sm_drag_full: 'Rack {k} is already there',
    sm_cm: 'cm',
    /* the camera */
    sm_cam: 'Camera', sm_cam_orbit: 'Orbit', sm_cam_walk: 'Walk',
    sm_fit_view: 'Fit', sm_top_view: 'Top', sm_front_view: 'Front',
    sm_fullscreen: 'Fullscreen', sm_exit_fullscreen: 'Exit fullscreen',
    sm_room_is_full: 'The room is fullscreen',
    sm_walk_hint: 'W A S D or the arrows to walk · drag to look around · click a shelf',
    sm_walk_hint_touch: 'Use the pads to walk and look · tap a shelf',
    sm_pad_fwd: 'Forward', sm_pad_back: 'Back', sm_pad_left: 'Left', sm_pad_right: 'Right',
    sm_pad_look: 'Look', sm_pad_toggle: 'Pads',
    sm_quality: 'Quality', sm_quality_auto: 'Auto', sm_quality_high: 'High', sm_quality_low: 'Low',
    sm_quality_note: 'Shadows change now; antialiasing on the next open of the map.',
    sm_quality_dropped: 'This machine draws the room slowly, so shadows are off here from now on.',
    /* rack size */
    sm_wall_cm: 'Position along the wall (cm from the left)',
    /* standing free on the floor (051) */
    sm_free: 'Free-standing',
    sm_free_x: 'From the left wall (cm, to its middle)',
    sm_free_y: 'From the front wall (cm, to its middle)',
    sm_rot: 'Turned', sm_rotate: 'Rotate a quarter turn',
    sm_free_hint: 'It stands on the floor and needs {n} cm of aisle to every wall and rack round it.',
    sm_the_wall_n: 'the back wall', sm_the_wall_e: 'the right wall', sm_the_wall_s: 'the front wall', sm_the_wall_w: 'the left wall',
    sm_drag_free: 'Free-standing, turned {r}° · {g} cm to {n}',
    sm_drag_rotate: 'R turns it',
    sm_drag_through: 'Through {w}',
    sm_drag_overlap: 'Into {n}',
    sm_err_overlap: 'Rack {k} overlaps {n}.',
    sm_err_aisle: 'Only {g} cm to {n} — needs {m} cm.',
    sm_err_through: 'Rack {k} would stand through {w}.',
    sm_err_free_unmeasured: 'A rack can stand free only in a measured room — give the room its width and depth first.',
    sm_err_rotation: 'A rack turns in quarter turns: 0°, 90°, 180° or 270°.',
    sm_err_placement: 'A rack stands on a wall or free on the floor — not both at once.',
    /* Stage C — the room is live, and racks are added and grown from inside it */
    sm_live_changed: 'The layout was changed on another screen.',
    sm_live_take: 'Show the new layout',
    sm_add_rack: 'Add a rack', sm_add_wall: 'On a wall',
    sm_add_place: 'Place it in the room',
    sm_add_hint: 'Move it into place and click to put it down. Esc cancels — nothing is saved before that.',
    sm_add_name: 'Rack {k}',
    sm_add_no_letter: 'Every letter A–Z is already a rack in this warehouse.',
    sm_grow_bays: '{n} bays', sm_grow_levels: '{n} levels',
    sm_grow_near: '{g} cm to {n}',
    sm_grow_ceiling: '{h} cm tall — the room is {r} cm high',
    sm_grow_wall: 'Past the end of {w}',
    sm_err_too_tall: 'Rack {k} would stand {h} cm tall and the room is {r} cm high — {m} levels at most.',
    sm_err_room_too_low: 'Rack {k} stands {h} cm tall — the room has to be at least that high.',
    sm_err_bay_occupied: 'Bay {b} holds {n} pieces — empty it first.',
    sm_err_level_occupied: 'Level {l} holds {n} pieces — empty it first.',
    sm_rack_size: 'Rack size',
    sm_rack_bay: 'Bay width (cm)', sm_rack_level: 'Level height (cm)', sm_rack_depth: 'Depth (cm)',
    sm_rack_size_hint: 'Left empty, the rack is the shop\'s standard: {b} cm bays, {l} cm levels, {d} cm deep.',
    sm_rack_size_hint_room: 'Left empty, the rack takes this room\'s size: {b} cm bays, {l} cm levels, {d} cm deep.',
    sm_shrink_preview: '{k} → {n} cm bays',
    sm_shrunk: 'Racks narrowed to fit the room',
    /* place my racks */
    sm_fill_racks: 'Place my racks',
    sm_fill_title: 'Place the racks along the walls',
    sm_fill_line: 'on the {w} at {n} cm',
    sm_fill_go: 'Place them',
    sm_fill_none_left: 'Every rack with shelves is already in a room.',
    sm_fill_partial: 'No room on the walls for: {k}. They stay unplaced.',
    /* the layout refusals */
    sm_err_wall_overlap: 'Rack {k} is already on that wall at {a}–{b} cm.',
    sm_err_wall_corner: 'Rack {k} stands in that corner.',
    sm_err_wall_short: 'Rack {k} is {w} cm wide and that wall has {n} cm left there.',
    sm_err_room_too_small: 'Rack {k} would not fit. This room needs at least {w} × {d} cm.',
    sm_err_room_too_small_short: '{k} needs {n} cm',
    sm_use_min: 'Use the minimum',
    sm_room_max: 'At most {n} m a side.',
    sm_drag_nofit: 'No free run on that wall',
    sm_drag_out: 'Outside the room',
    sm_overflow: '{k} runs {m} cm past the {w}',
    sm_room_size: '{w} × {d} m',
    sm_other_rooms: 'In the warehouse\'s other rooms',
    sm_no_webgl: 'This machine cannot draw the 3D room — showing the flat map instead.',
    sm_gl_lost: 'The 3D room stopped on this machine — back to the flat map.',
    sm_colour_by_type: 'Colour by type',
    sm_view: 'View', sm_view_2d: 'Plan', sm_view_room: '3D room',
    sm_room_off_motion: 'The room needs motion, and this machine has asked for less.',
    sm_remove_level: 'Remove level {x}', sm_remove_bay: 'Remove bay {x}',
    sm_flat_hint: 'Click a rack in the plan to open it · point at a bay to see what is on it',
    sm_rack_empty: 'This rack has no shelves yet.',
    sm_design: 'Design the room',
    sm_racks_here: 'Racks in this room', sm_no_racks: 'No racks in this room yet',
    sm_new_rack: 'New rack', sm_rack_cfg: 'Rack settings', sm_rack_key: 'Rack letter',
    sm_rack_name: 'Rack name', sm_delete_rack: 'Delete rack',
    sm_room_settings: 'Room settings', sm_not_placed: 'Not placed in a room yet',
    sm_place: 'Place', sm_nowhere: '— not placed —',
    sm_wall: 'Wall', sm_wall_n: 'Back wall', sm_wall_e: 'Right wall', sm_wall_s: 'Front wall', sm_wall_w: 'Left wall',
    sm_levels: 'Levels', sm_bays: 'Bays', sm_add_level: 'Add level', sm_add_bay: 'Add bay',
    sm_level_hint: 'A is the top level; a new one goes at the bottom.',
    sm_room_w: 'Width (cm)', sm_room_d: 'Depth (cm)', sm_room_h: 'Height (cm)',
    sm_measure_hint: 'Optional. Left empty, the room is sized to its racks and marked "not to scale".',
    sm_not_to_scale: 'not to scale',
    sm_racks_first: 'Add a room, then racks into it.',
    ty_sneakers: 'Sneakers', ty_boots: 'Boots', ty_tshirts: 'T-Shirts', ty_jeans: 'Jeans',
    ty_jerseys: 'Jerseys', ty_crocs: 'Crocs', ty_shirts: 'Shirts', ty_jackets: 'Jackets',
    sm_new_room: 'New room', sm_delete_room: 'Delete room',
    sm_room_name: 'Room name, in your words',
    sm_origin: 'Column 1 starts on the', sm_origin_left: 'Left', sm_origin_right: 'Right',
    sm_origin_hint: 'As you stand in that room\'s doorway. The map matches the room, not the menu language.',
    sm_grid_setup: 'Lay out the grid',
    sm_capacity: 'Capacity (boxes)', sm_cap_hint: 'leave empty if unknown',
    sm_capacity_n: 'holds {n}',
    sm_remove_n: '{n} shelf/shelves go, {a} of them assigned. Stock blocks removal.',
    sm_remove_go: 'Remove',
    sm_no_renumber: 'Removing a row or column never renumbers the rest — every label already printed stays true.',
    sm_unassigned: 'Unassigned — takes whatever is put on it first',
    sm_unassigned_opt: '— unassigned —',
    sm_whole_model: 'Whole model',
    sm_shelf_empty: 'Nothing on this shelf.',
    /* the room: the floor inside the door, and a bay pressed in 3D */
    sm_front_area: 'Front area',
    sm_bay_where: 'Rack {r} · Level {l} · Bay {b}',
    sm_bay_total: '{n} pieces',
    sm_bay_empty: 'Nothing here yet — scan a pair to put it on {code}.',
    sm_bay_more: '+{n} more sizes',
    sm_bay_back: 'Esc, or press it again, to step back',
    sm_code: 'Code', sm_delete: 'Delete shelf',
    sm_locked: '{n} pc(s) on it — it cannot be deleted',
    sm_confirm_title: 'This shelf is not empty',
    sm_confirm_stock: '{n} pc(s) of {p} are sitting on it right now.',
    sm_confirm_code: 'Renaming the code makes this shelf\'s printed label wrong.',
    sm_stale: '{n} printed label(s) become stale',
    sm_reprint: 'Reprint labels', sm_apply: 'Apply anyway',
    sm_print_label: 'Print shelf label',
    sm_target: 'Putting away onto {code}',
    sm_scan_hint: 'scan the pairs going onto it',
    sm_no_home: 'That model has no shelf yet',
    sm_shelf_gone: 'No such shelf on the map',
    sm_unknown_code: 'Not a shelf or product code',
    sm_shelves_title: 'Shelf assignments',
    sm_shelves_sub: 'Which model each shelf is for. The map itself is under Shelf map.',
    sm_open_map: 'Open the map',
    nav_shelfmap: 'Shelf map',
    sm_scan_ph: 'Scan a shelf, then scan pairs…',
    sm_no_target: 'No shelf picked',
    sm_no_target_hint: 'Scan a shelf label to start putting away, or scan a pair to find where it goes',
    sm_target_set: 'target set',
    sm_refused: 'refused',
    sm_last_scans: 'Last scans',
    sm_undo_hint: 'Tap to undo this scan',
    sm_clear_strip: 'Clear strip',
    sm_sound_on: 'Sound on', sm_sound_off: 'Sound off',

    /* --- discounts --- */
    disc_capped: 'Discounts above {p}% need a manager',
    disc_max: 'most you can take off: {v}',
    wh_cost_later: 'A manager adds the cost price. Enter what the shop sells it for.',

    dash_title: 'Dashboard', dash_sub: 'Everything about the shop, on one screen',
    st_today: 'Sales today', st_products: 'Products',
    st_critical: 'SKUs in critical stock', st_customers: 'Active customers', st_print: 'Pending print jobs',
    vs_last_month: 'vs last month',
    in_catalogue: 'in the catalogue',
    sales_6m: 'Sales — last 6 months', sales_by_type: 'Sales by product type',
    needs_attention: 'Needs attention', recent_sales: 'Latest sales', fix: 'Fix',

    invoice: 'Invoice', customer: 'Customer', date: 'Date', items: 'Items', total: 'Total',
    payment: 'Payment', cashier: 'Cashier', actions: 'Actions', qty: 'Qty', size: 'Size',
    unit_price: 'Unit price', line_total: 'Line total', price: 'Price', cost: 'Cost',
    margin: 'Margin', stock: 'Stock', type: 'Type', product: 'Product', name: 'Name',
    phone: 'Phone', city: 'City', status: 'Status', notes: 'Notes', barcode: 'Barcode',
    shelf: 'Shelf', sku: 'SKU', brand: 'Brand', made_in: 'Made in', colour: 'Colourway',

    pos_title: 'Cashier',
    scan_ph: 'Scan barcode or search product…', scan_btn: 'Scan',
    cart: 'Cart', clear: 'Clear', empty_cart: 'Cart is empty',
    empty_cart_sub: 'Scan a barcode or tap a product to start a sale',
    all_products: 'All', subtotal: 'Subtotal', discount: 'Discount', coupon: 'Coupon code',
    apply: 'Apply', applied: 'Applied', points: 'points', loyalty: 'Loyalty',
    use_points: 'Use', payment_method: 'Payment method', add_print: 'Add print job to this sale',
    complete_sale: 'Complete sale', customer_ph: 'Pick a customer, or type to search…', change_customer: 'Change',
    walk_in: 'Walk-in customer', priority: 'Priority',
    deadline: 'Deadline', normal: 'Normal', urgent: 'Urgent', amount: 'Amount', percent: 'Percent',
    in_stock: 'in stock', pick_size: 'Pick a size', out_of_stock: 'Out of stock',
    sale_complete: 'Sale complete', points_earned: 'Loyalty points earned',
    new_sale: 'New sale', print: 'Print', pdf: 'PDF', thank_you: 'Thank you — see you soon',
    served_by: 'Served by', bill_to: 'Bill to',

    products_title: 'Products', products_sub: 'Every item, every size, live stock',
    health: 'Stock health', visible: 'On website', healthy: 'Healthy', low: 'Low',
    critical: 'Critical', out: 'Out', all_types: 'All types', all_health: 'All health',
    per_size: 'Stock per size', sales_trend: 'Sales trend — last 6 months',
    size_gap_warn: 'Size gap — good total stock, zero in the sizes people ask for',
    total_stock: 'Total stock', stock_value: 'Stock value at cost', last_sold: 'Last sold',
    ms_title: 'Move by scan',
    ms_swap: 'Swap the two places',
    ms_scan_hint: 'Scan a barcode, or type a SKU and press Enter',
    ms_camera: 'Camera',
    ms_empty: 'Nothing on the list yet',
    ms_empty_sub: 'Scan each piece you are carrying. Nothing moves until you press the button below.',
    ms_available: 'There now', ms_moving: 'Moving',
    ms_move_n: 'Move {n} pieces',
    ms_on_list: '{n} on the list',
    ms_none_at: 'There are none in {place}',
    ms_over: '{n} line(s) ask for more than that place holds \u2014 only what is there will move.',
    ms_note: 'Carried to the floor',
    ms_some_failed: '{n} did not move',
    wh_moves_note: 'Every movement is stamped with a user, a date and the balance after it. Rows cannot be deleted — only corrected with a new movement.',
    edit_product: 'Edit product', reorder: 'Create reorder',
    bk_delete: 'Delete',
    bk_del_confirm: 'Delete {n} product(s) for good?\n\n{names}\n\nThis cannot be undone. Anything that has been sold, moved, ordered or counted will be kept and named instead — archive those.',
    bk_deleted: '{n} deleted',
    bk_del_kept: '{n} could not be deleted',
    pe_note: 'Sizes and stock are not edited here: book stock in and out through the warehouse, so every change is on the movement log.',
    name_required: 'A name is required.', price_required: 'A selling price is required.',
    pe_saved: 'Saved',

    warehouse_title: 'Warehouse', warehouse_sub: 'Nothing moves without a trace',
    tab_add: 'Add product', tab_moves: 'Stock movements',

    /* Multi-warehouse. The wording avoids "location" as a noun on buttons —
       "Shop floor" and "Back storage" are what the staff actually say. */
    wh_stock: 'Stock by place', wh_all: 'Everywhere',
    wh_location: 'Place', wh_sell_from: 'Take the stock from',
    wh_not_here: 'Not on the shop floor', wh_in_the_back: 'in the back',
    wh_bring_out: 'Bring one out', wh_moved: 'Moved',
    wh_count_where: 'Which place are you counting?',
    wh_transfer: 'Move stock', wh_move: 'Move', wh_from: 'From', wh_to: 'To',
    wh_qty_to_move: 'How many', wh_move_done: 'Stock moved',
    wh_suggest: 'Bring these out of the back',
    wh_suggest_sub: 'Empty on the shop floor, sitting in storage',
    wh_here: 'Here',
    wh_pieces_here: 'pieces here', wh_value_here: 'Value at cost',
    wh_skus_here: 'sizes stocked here',

    /* ---- the order handshake with Yalla Wear ----------------------------
       Wording is careful about who is doing what. OG *offers* an order; only
       Yalla can accept it. Nothing here may imply OG put the job on the press. */
    or_state: 'Order', or_draft: 'Not sent yet', or_pending: 'Waiting on Yalla Wear',
    or_accepted: 'Accepted', or_declined: 'Declined',
    or_send: 'Send to Yalla Wear', or_send_again: 'Send again',
    or_send_title: 'Send this order to Yalla Wear',
    or_send_hint: 'This is exactly what Yalla Wear will see. They can accept it or turn it down.',
    or_sent_ok: 'Order sent — waiting for Yalla Wear to accept',
    or_cannot: 'Cannot send this order',
    or_why_tbc: 'Some shirts still have no name. Yalla Wear cannot print a blank back — confirm the names first.',
    or_why_sent: 'This order is already with Yalla Wear.',
    or_why_accepted: 'Yalla Wear has already accepted this one.',
    or_wait_head: 'waiting on Yalla Wear',
    or_wait_sub: 'Sent, with no answer yet',
    or_declined_head: 'Yalla Wear turned this down',
    or_promised: 'Promised by', or_requested: 'You asked for',
    or_timeline: 'Order timeline',
    or_tl_created: 'Job created', or_tl_sent: 'Sent to Yalla Wear',
    or_tl_accepted: 'Accepted by Yalla Wear', or_tl_declined: 'Declined by Yalla Wear',
    or_by_og: 'OG', or_by_yalla: 'Yalla Wear',
    or_blocked_stage: 'Yalla Wear has not accepted this order yet',
    or_wa: 'Send on WhatsApp too',
    or_wa_title: 'Send the order on WhatsApp',

    /* partner side */
    yl_new_orders: 'New orders', yl_new_orders_sub: 'Sent by OG, waiting on you',
    yl_accept: 'Accept', yl_decline: 'Turn down',
    yl_accept_title: 'Accept this order', yl_decline_title: 'Turn down this order',
    yl_promise: 'Ready by', yl_promise_hint: 'This is the date you will be measured on, not the one OG asked for.',
    yl_decline_hint: 'Tell OG why, in your own words. They will see this straight away.',
    yl_decline_ph: 'e.g. the press is booked solid until Sunday…',
    yl_accepted_ok: 'Accepted — it is now on your board',
    yl_declined_ok: 'Turned down — OG has been told',
    yl_requested_by: 'OG asked for',
    yl_sent_ago: 'Sent',
    wh_empty_sizes: 'sizes empty here', wh_none_here: 'Nothing in this place',
    wh_nothing_to_move: 'Nothing needs bringing out — the floor is stocked',
    wh_split_hint: 'Shop floor / Back storage',
    image: 'Image', product_name: 'Product name',
    /* Product photo intake. Three ways in, so the hint names all three. */
    nav_more: 'More', language: 'Language',
    /* WhatsApp. The wording is careful: the app OPENS WhatsApp, it cannot
       send on the user's behalf, and the UI must not imply otherwise. */
    wa_open: 'Open in WhatsApp', wa_opened: 'WhatsApp opened with the message ready',
    wa_handoff: 'This opens WhatsApp with the message written. You press send there.',
    wa_bad_number: 'That phone number is not usable',
    wa_day_title: 'Send today’s summary', wa_day_empty: 'No sales recorded today yet',
    wa_send_day: 'Send the day',
    /* purchase orders */
    po_place: 'Place the order', po_rate: 'Sells', po_week: 'wk', po_cover: 'Lasts',
    po_order: 'Order', po_no_sales: 'not selling',
    po_need_qty: 'Set a quantity on at least one size',
    po_explain: 'Quantities are suggested from what actually sold in the last 8 weeks, ' +
                'enough to cover the next 4. Change anything you like.',
    po_whatsapp: 'Send on WhatsApp', po_received_toast: 'received into stock',
    po_title: 'Purchase orders', po_draft: 'Draft', po_sent: 'Sent', po_received: 'Received',
    tab_reorder: 'Worth reordering',
    po_open: 'Open orders',
    po_receive: 'Receive stock', po_none: 'No purchase orders yet',
    po_none_sub: 'Reorder any low product and it appears here',
    po_suggest: 'Worth reordering', po_suggest_sub: 'from real sales speed, most urgent first',
    /* scanning */
    sc_title: 'Scan', sc_looking: 'Point the camera at the label',
    sc_found: 'Found', sc_found_title: 'Scanned', sc_again: 'Scan another',
    sc_photo: 'Take a photo instead', sc_torch: 'Light',
    sc_no_torch: 'This camera has no light',
    sc_type_code: 'or type the barcode', sc_go: 'Go',
    sc_reading: 'Reading the photo…', sc_nothing: 'No code found in that photo',
    sc_unknown: 'Nothing in the system matches',
    sc_denied: 'Camera permission was refused — allow it, or use a photo',
    sc_nodevice: 'No camera found on this device',
    sc_failed: 'The camera could not be opened',
    sc_no_camera: 'Live camera needs https',
    sc_no_camera_sub: 'Opened from a file, the browser blocks the camera. Take a photo instead — it works the same.',
    sc_use_photo: 'Take a photo of the label',
    sc_this_size: 'Scanned size', sc_all_sizes: 'Every size', sc_scanned: 'scanned',
    sc_per_week: 'a week',

    /* ---- what a scan can do, and the hardware behind it -----------------
       Wording is deliberate: "check in" is putting stock away, "check out" is
       taking it off the shelf for any reason that is not a sale. Selling is
       its own word because it is the only one that takes money. */
    sc_sells: 'Selling', sc_last_8w: 'over the last 8 weeks',
    sc_months: 'mo', sc_over_a_year: 'over a year',
    sc_cover: 'Lasts about', sc_at_this_rate: 'at this speed',
    sc_not_moving: 'not moving', sc_never_sold: 'never sold in this size',
    sc_sourcing: 'Where it comes from', sc_last_delivery: 'Last delivery',
    sc_no_delivery: 'nothing recorded yet',
    sc_recent_moves: 'Recent movements', sc_no_moves: 'Nothing has moved on this size yet.',
    sc_what_now: 'What are you doing with it?',
    sc_check_in: 'Put into stock', sc_check_out: 'Take out of stock',
    sc_sell: 'Sell it',
    sc_enter_hint: 'Press Enter to sell straight away — the scanner can stay in your hand.',
    sc_checked_in: 'Put into stock', sc_checked_out: 'Taken out of stock',
    sc_in_note: 'Checked in by scan', sc_out_note: 'Checked out by scan',
    sc_cannot_out: 'Nothing to take out', sc_only_had: 'only had',

    hw_title: 'Scanner and printer', hw_sub: 'The hardware, and how to prove it works',
    hw_scanner: 'Barcode scanner',
    hw_scanner_note: 'A cable, dongle or Bluetooth scanner all type like a keyboard, so all three work with no setup. Scan into the box below to check yours.',
    hw_test: 'Scan anything here to test', hw_waiting: 'Waiting for a scan…',
    hw_last: 'Last scan', hw_gap: 'Speed', hw_accepted: 'Recognised as a scan',
    hw_rejected: 'Read as typing, not a scan',
    hw_prefix: 'Prefix character', hw_prefix_note: 'Optional. If you program your scanner to send a character first, recognition becomes exact instead of based on speed.',
    hw_threshold: 'Speed limit', hw_threshold_note: 'A scanner types faster than this many milliseconds per character. Raise it if real scans are being missed.',
    hw_printer: 'Label printer',
    hw_printer_note: '“This computer” output needs the label printer installed in Windows as a printer; the queue output needs the print agent or the printer’s LAN address (Labels fold below). Print a test label, then the ruler.',
    hw_mode: 'Paper', hw_roll: 'Label roll', hw_sheet: 'A4 sticker sheet',
    hw_test_label: 'Print a test label', hw_calibrate: 'Print a ruler',
    hw_calibrate_note: 'The printed rulers must measure exactly 10mm. If they do not, the printer is scaling and the label size in its driver does not match the roll.',
    hw_sym_note: 'The bars carry the shop’s own label code (or the EAN-13 where the sticker is wide enough) — the same on every screen, and a scan of either finds the size.',
    hw_camera_gap: 'On a phone without native barcode support the camera reads EAN-13 only, not Code 128. A hardware scanner is unaffected — it decodes on the device.',
    sc_running_out: 'Runs out in about {d} days',
    sizes: 'Sizes',
    /* stock count */
    st_count: 'Stock count', st_intro_sub: 'Walk the shelf, scan, compare',
    st_ready: 'Ready to count',
    st_ready_sub: 'Scan or type what is actually on the shelf. Nothing changes until you post — ' +
                  'the count is compared against the system first, and you see every difference before it applies.',
    st_started: 'started', st_progress: 'Counted', st_variance: 'Differences',
    st_sizes_differ: 'sizes do not match', st_net_pieces: 'Net pieces', st_vs_system: 'against the system',
    st_value: 'Value', st_at_cost: 'at cost price',
    st_system: 'System', st_counted: 'Counted', st_diff: 'Diff',
    st_f_all: 'All', st_f_variance: 'Differences only', st_f_counted: 'Counted', st_f_uncounted: 'Not counted',
    st_find: 'Find a product, SKU or barcode…',
    st_now_counted: 'counted:', st_showing: 'Showing {n} of {t} — use the search to narrow it down',
    st_post: 'Post the count', st_post_q: 'Adjust {n} sizes?',
    st_post_sub: '{p} pieces · {v} at cost',
    st_post_note: 'Only sizes you actually counted are touched. Anything left blank is untouched, ' +
                  'and every adjustment is written into the stock movement log.',
    st_posted: 'posted — {n} sizes adjusted, {p} pieces',
    st_adjust_note: 'Stock count', st_last: 'Last count', st_adjusted: 'Sizes adjusted',
    st_discard_title: 'Discard this count?',
    st_discard_body: 'You have counted {n} sizes. Nothing has been applied to stock yet, and closing now loses the count.',
    st_discard: 'Discard it',
    /* duplicate guard */
    dup_title: 'This may already be in the catalogue',
    dup_head: 'A very similar product already exists',
    dup_sub: 'Adding it twice splits the stock between two products, and neither count is right.',
    dup_match: 'Match', dup_use: 'Open this one instead',
    dup_anyway: 'It is a different product — add it',
    dup_note: 'You are adding “{n}”. If it is genuinely a different colourway or model, carry on.',
    mo_title: 'Motion', mo_animations: 'Animations and transitions',
    mo_hint: 'Turn this off if the app feels slow on a projector or an old phone.',
    mo_on: 'animations on', mo_off: 'animations off',
    /* command palette */
    cp_title: 'Quick search', cp_placeholder: 'Search or type a command…',
    cp_new_sale: 'New sale', cp_add_product: 'Add a product',
    cp_scan_hint: 'camera', cp_go_to: 'go to',
    cp_toggle_lang: 'Switch language', cp_toggle_curr: 'Switch currency',
    cp_cmd: 'action', cp_product: 'product', cp_customer: 'customer',
    cp_invoice: 'invoice', cp_job: 'print job',
    /* money */
    nav_money: 'Money', mn_title: 'Money', mn_sub: 'The drawer, the costs, and who owes you',
    mn_shift: 'Shift', mn_expenses: 'Expenses', mn_debt: 'Debt book',
    mn_open_shift: 'Open a shift', mn_close_shift: 'Close the shift',
    mn_no_shift: 'No shift is open',
    mn_no_shift_sub: 'Open a shift with the cash you are starting with. Every sale gets attached to it, ' +
                     'and at closing the system tells you what should be in the drawer.',
    mn_cashier: 'Cashier', mn_float: 'Opening float',
    mn_float_hint: 'The cash already in the drawer before the first sale.',
    mn_shift_open: 'shift open', mn_open_since: 'Open since',
    mn_sales: 'Sold this shift', mn_in_drawer: 'Should be in the drawer',
    mn_expected_now: 'right now', mn_not_drawer: 'Not in the drawer',
    mn_settles_later: 'Sham Cash, card — settles later',
    mn_drawer_now: 'The drawer', mn_by_method: 'How they paid',
    mn_cash_sales: 'Cash sales', mn_debt_settled: 'Debt settled', mn_cash_out: 'Paid out in cash',
    mn_expected: 'Expected', mn_counted: 'Counted', mn_difference: 'Difference',
    mn_count_now: 'Count the drawer and type the total',
    mn_close_hint: 'Count the notes before you look at the expected figure — ' +
                   'checking against a number you already know is not a count.',
    mn_count_needed: 'Type what you counted', mn_closed: 'closed',
    mn_balanced: 'Balanced exactly', mn_short: 'short', mn_over: 'over',
    mn_last_shift: 'Last shift', mn_in_box: 'in the box', mn_to_account: 'to an account',
    mn_owed: 'owed to you',
    mn_no_sales_yet: 'No sales yet this shift', mn_no_sales_sub: 'They appear here as they happen',
    mn_add: 'Add', mn_add_expense: 'Add an expense', mn_amount: 'Amount',
    mn_amount_needed: 'Enter an amount', mn_where: 'Where the money goes',
    mn_category: 'Category',
    mn_exp_month: 'Spent this month',
    mn_net: 'Real profit',
    mn_c_rent: 'Rent', mn_c_generator: 'Generator / fuel', mn_c_salaries: 'Salaries',
    mn_c_transport: 'Transport', mn_c_packaging: 'Packaging', mn_c_supplier: 'Supplier',
    mn_c_other: 'Other',
    mn_owed_total: 'Owed to you', mn_people: 'unpaid invoices', mn_over_30: 'Older than 30 days',
    mn_chase_these: 'worth chasing', mn_oldest: 'Oldest', mn_still_owed: 'Still owed',
    mn_age: 'Age', mn_settle: 'Settle', mn_remind: 'Remind',
    mn_part_paid: 'part paid, still', mn_cleared: 'cleared',
    mn_settle_hint: 'Cash settled while a shift is open goes into that drawer.',
    mn_no_debt: 'Nobody owes you anything', mn_no_debt_sub: 'Credit sales appear here until they are paid',
    up_pick: 'Add a photo', up_hint: 'click · drag · paste',
    up_swap: 'Change photo',
    up_ok: 'Photo added',
    img_change: 'Change the picture', img_add: 'Add a picture', img_remove: 'Remove', img_removed: 'Picture removed',
    img_open: 'Open the picture', img_uploading: 'Sending the picture to the cloud…',
    img_saved: 'Picture saved — it now shows everywhere this product does.',
    img_fail: 'The product is saved, but the picture did not reach the cloud. Add it later from the product drawer.',
    img_stale_server: 'The product is saved. The server is running older code and does not know how to store pictures yet — press Hard refresh in OG System, then add the picture from the product drawer.',
    up_err_none: 'No file was selected',
    up_err_type: 'That is not an image file',
    up_err_size: 'That photo is too large — 12 MB is the limit',
    up_err_read: 'The file could not be read',
    up_err_decode: 'That image is damaged or in a format the browser cannot open',
    cost_price: 'Cost price', selling_price: 'Selling price', shelf_box: 'Shelf / box',
    wh_intake: 'Received into',
    shelf_none: '— not put away yet —',
    shelf_no_rooms: 'No shelves in this place yet',
    wh_shelf_partial: '{n} of {m} sizes were not put on the shelf.',
    size_matrix: 'Quantity per size', matrix_hint: 'Enter a quantity per size — the codes are issued when you save',
    print_labels: 'Print barcode labels', save_product: 'Save product to warehouse',
    movement: 'Movement', user: 'User',
    balance: 'Balance after', received: 'Received', sold: 'Sold', damaged: 'Damaged',
    returned: 'Return', transfer: 'Transfer',
    total_pieces: 'Total pieces', total_cost: 'Total cost', expected_revenue: 'Expected revenue',

    customers_title: 'Customers', customers_sub: 'Who buys, what they buy, when they stopped',
    cu_new: 'New customer', cu_name_ph: 'Full name',
    cu_more: '{n} more — keep typing to narrow it down',
    cu_new_note: 'Three fields is all it takes. Points start at zero and build from the first sale.',
    cu_exists: 'Already in the list',
    no_access: 'Your account does not have access to this',
    tier: 'Tier', total_spent: 'Total spent', last_purchase: 'Last purchase',
    send_whatsapp: 'Send WhatsApp', purchase_history: 'Purchase history',
    points_timeline: 'Loyalty timeline', preferred_sizes: 'Preferred sizes',
    source: 'Source', online: 'Online', in_store: 'In-store', gold: 'Gold', silver: 'Silver',
    bronze: 'Bronze', all_customers: 'All customers',
    gold_only: 'Gold only', whatsapp_msg: 'WhatsApp message', send: 'Send',
    days_ago: 'days ago', today_word: 'today', yesterday: 'yesterday', in_days: 'in',
    days: 'days', orders: 'Orders',
    cu_last_in: 'Last in', cu_never_bought: 'Not bought yet', cu_quiet: 'Gone quiet',
    cu_quiet_only: 'Gone quiet', cu_owes_only: 'Owes money',
    cu_rhythm: 'Comes in', cu_every_n: 'every {n} d', cu_quiet_after: 'quiet after {n} d',
    cu_rhythm_unknown: 'Needs three purchases to tell',
    cu_none_size: 'Nobody on file wears {n}',
    cu_none_size_sub: 'Sizes come from what people have actually bought, so a new size shows up after its first sale.',
    cu_clear_size: 'Show everyone',
    cu_none_search: 'No one by that name or number',
    cu_none_search_sub: 'Arabic and Latin spellings both match, and a phone matches in any format. If they are genuinely new, add them.',
    cu_none_debt: 'Nobody owes you anything',
    cu_none_debt_sub: 'Credit sales appear here until they are paid off.',
    cu_none_quiet: 'Everybody is on schedule',
    cu_none_quiet_sub: 'People show up here when they go past their own usual gap between visits.',
    cu_none_archived: 'Nothing archived',
    cu_none_archived_sub: 'Archived customers keep all their invoices — they are only hidden from the list.',
    cu_none_gold: 'No gold customers yet',
    cu_none_gold_sub: 'Points build with every purchase; the first regulars will arrive here on their own.',
    cu_none_at_all: 'No customers yet',
    cu_none_at_all_sub: 'Add the first one, and the shop starts remembering who buys what.',
    cu_gone: 'No such customer',
    cu_gone_sub: 'This link may be old, or the account signed in cannot see this person.',
    cu_timeline: 'Everything, in order', cu_events: 'events',
    cu_tl_empty: 'Nothing has happened yet',
    cu_tl_empty_sub: 'Purchases, points and deliveries all appear here as they happen.',
    cu_from_invoice: 'earned on {n}', cu_spent_on: 'spent on {n}',
    cu_details: 'Details', cu_no_address: 'No address on file', cu_no_note: 'No note',
    cu_no_sizes: 'Nothing bought yet, so no sizes to go on',
    cu_size_drift: 'They used to buy {a} and now buy {b} — either their size changed, or they are buying for somebody else.',
    cu_open_profile: 'Open the full record',
    cu_edit: 'Edit customer', cu_addr_ph: 'street, building, flat',
    cu_note_ph: 'anything worth remembering',
    cu_note_seen: 'The note is visible to anyone who can see customers.',
    cu_archive_ask: 'Archive {n}?',
    cu_archive_note: 'Nothing is deleted. Their invoices stay exactly as they are, and the Archived filter brings them back.',
    address: 'Address', note: 'Note', edit: 'Edit', dl_one: 'Delivery', by: 'By',
    messages: 'Messages', currency: 'Currency',
    /* Six keys that were used but never defined — found by the sweep in the
       Stage D report. Each rendered its own key name inside an Arabic RTL
       layout, which is exactly what the en/ar discipline exists to stop. */
    if_sold_all: 'if everything sells', bulk_title: 'Bulk action', undo: 'Undo',
    warehouse: 'Warehouse', print_done: 'Done', yi_title: 'Partner invoice',
    cap_window: 'Showing the most recent {a} of {b} {n} — every figure on this screen is of those {a}, not of the whole shop.',
    cap_of: 'of {b} in total', rows: 'rows',
    nt_showing: 'Showing the newest {a} of {b} messages.',
    cu_tl_more: 'Show {n} older events',
    cu_card: 'Loyalty card', cu_card_already: '{n} is already on this sale',
    cu_card_swapped: 'swapped in',
    hw_roll_claim: 'what the app believes one sticker is',
    hw_roll_note: 'The four corner marks sit at the edges of the sticker the app expects. If they land on the corners of the real sticker, the size above is right. If they fall inside it or run off it, that size is what needs changing — measure the sticker and tell us the millimetres.',
    cu_take_payment: 'Take a payment', cu_owed: 'Still owed',
    cu_then_now: 'Then / now', cu_then: 'then', cu_now: 'now',
    cu_then_now_note: 'The dollar figures are approximate — the "then" column uses the rate frozen on each sale, the "now" column today\'s. Neither is what is owed; the amount owed is the figure in the left column.',
    cu_part_paid: '{n} paid so far', cu_debt_settled: 'Everything is paid off.',
    cu_pay_note: 'This is money into the drawer, in this shift. It cannot be taken back by doing it again, so the amount is checked against what is actually still owed.',
    cu_pay_amount_needed: 'Enter an amount', cu_debt_cleared: 'settled in full',
    cu_still_owed: '{n} still owed',
    cu_credit_limit: 'Credit limit ($)', cu_no_limit: 'no limit',
    cu_no_credit: 'Credit', cu_credit_ok: 'Credit allowed', cu_credit_none: 'No credit',
    cu_credit_note: 'The limit is in dollars, so it means the same next year as it does today. Leave it blank for no limit — 0 means no credit at all. Going over warns; the No credit setting refuses.',
    cu_limit_of: 'limit {n}',
    cu_over_limit: 'Over their credit limit',
    cu_over_limit_msg: '{n} now owes about {a}, against a limit of {b}. The sale went through.',
    cu_merge: 'Merge duplicates', cu_merge_pick: 'This one',
    cu_merge_ask: 'Which record is the same person as {n}?',
    cu_merge_which: 'The record to merge IN',
    cu_merge_note: 'The record you are on now survives. The other one is archived with a pointer to this one — nothing is deleted, and every invoice keeps the name it was rung up under.',
    cu_merge_no_obvious: 'No obvious duplicate',
    cu_merge_no_obvious_sub: 'Nobody else has the same name or number. Search to pick one anyway.',
    cu_merge_confirm: 'Merge {a} into {b}?',
    cu_merge_sales: 'Their {n} purchases move across, with the points those earned.',
    cu_merge_points: 'Points add up: {a} + {b} = {c}.',
    cu_merge_archived: '{n} is archived, not deleted.',
    cu_merge_irreversible: 'There is no undo button for this.',
    cu_merged: '{n} purchases moved across',
    cu_merged_into: 'This record was merged into {n}.',
    pj_link: 'Link a customer', pj_link_ask: 'Who is job {n} for?',
    pj_link_note: 'Only link somebody you are sure of. A job matched by name across two spellings attaches the wrong person\'s order, which is why nothing was guessed for you.',
    wa_none_sub: 'When a size is looked up at the till while it is out of stock, whoever asked is recorded here automatically.',
    wa_note: 'Nobody typed any of this — each row was recorded when somebody asked for that size at the till and it was not in stock. A row turns green once the size is back.',
    wa_told: 'Marked as told',
    sa_attach: 'Attach a customer', sa_pick: 'Attach',
    sa_ask: 'Who was {n} for?',
    sa_note: 'The points this sale would have earned are added now, at the rate the sale was rung up at. The name printed on the invoice does not change — it is a record of that moment.',
    wa_wanted: 'Asked for {n}', wa_answered: 'we came back to them',
    wa_still_waiting: 'still waiting', wa_wants: 'Asked for and not in stock',
    wa_none: 'Nothing outstanding',
    wa_tell: 'Mark as told',
    pr_wear: 'wear this', pr_wearers_hint: 'Customers on file who buy this size — open the list',
    dl_failed_before: '{n} earlier delivery did not arrive',
    ly_card: 'Stamp card', ly_stamps: 'stamps', ly_full: 'Card full — reward owed',
    ly_to_go: '{n} more to go', ly_full_only: 'Card full',
    ly_owed_many: 'They have filled {n} cards and not cashed any of them in yet.',
    ly_past: 'Cashed in before', ly_used_n: '{n} of {r} stamps',
    ly_redeem: 'Cash the card in', ly_redeemed: 'Card cashed in · {n} stamps',
    ly_carried: '{n} carried over', ly_no_note: 'nothing recorded',
    ly_redeem_ask: 'Cash in a full card for {n}?',
    ly_given: 'What did they get?', ly_given_ph: 'a free pair, 20% off, early access…',
    ly_redeem_note: 'Nothing is worked out automatically — you decide what the card is worth, and this records what was actually handed over. Any spare stamps carry over to the next card.',
    ly_mode: 'What the shop runs', ly_mode_points: 'Points only', ly_mode_stamps: 'Stamp cards only',
    ly_mode_both: 'Points and stamp cards', ly_mode_off: 'Neither',
    ly_block: 'Points redeemed at once',
    ly_required: 'Stamps to fill a card', ly_per: 'A stamp is earned per',
    ly_per_item: 'item bought', ly_per_visit: 'visit',
    ly_stamps_note: 'Stamps are counted from what people actually bought, so voiding a sale takes its stamps back on its own. Nothing is stored, and nothing can drift.',
    cu_visits: 'Visits', cu_debt: 'Owes', cu_since: 'Customer since',
    cu_voided: 'Voided',
    cu_sort: 'Sort', cu_sort_recent: 'Recent first', cu_sort_name: 'Name',
    cu_sort_spend: 'Spend', cu_sort_visits: 'Visits', cu_sort_debt: 'Debt',
    cu_showing: 'Showing {a} of {b} — narrow the search to see the rest',
    cu_phone_taken: 'Saved — but {n} already has this number',
    cu_phone_taken_archived: 'Saved — but {n} (an archived customer) already has this number',
    cu_view: 'Open',
    cu_atrisk_title: 'At-risk window',
    cu_atrisk_days: 'Days without a purchase before a customer counts as at risk',
    cu_atrisk_note: 'Moves the red badge, the At-risk filter and every at-risk count, for every account. Saved to the server.',
    fam_Footwear: 'Footwear', fam_Jeans: 'Jeans', fam_Tops: 'Tops',

    print_title: 'Print Jobs', print_sub: 'Every t-shirt job, from design to delivery',
    jobs_month: 'Jobs this month', on_time: 'On-time', print_revenue: 'Printing revenue',
    paid_partner: 'Paid to Yalla Wear',
    partner_access: 'Partner access',
    partner_note: 'This is everything the printing partner can see. No costs, no customer phone numbers, no stock, no prices you charge.',
    overdue: 'Overdue', design_note: 'Design note', drag_hint: 'Drag a card between columns to move the job',

    reports_title: 'Reports', reports_sub: 'The numbers behind the shop',
    tab_sales: 'Sales', tab_profit: 'Profit', tab_inventory: 'Inventory value',
    tab_employees: 'Employees', tab_suppliers: 'Suppliers',
    export_excel: 'Export Excel', export_pdf: 'Export PDF', revenue: 'Revenue',
    profit: 'Profit', units: 'Units', capital_in_stock: 'Capital sitting in stock',
    retail_value: 'Retail value', role: 'Role', salary: 'Monthly salary',
    next_payment: 'Next payment', outstanding: 'Outstanding', due: 'Due',
    supplier: 'Supplier', category: 'Category', sales_made: 'Sales handled',
    avg_basket: 'Average basket', invoices: 'Invoices', best_sellers: 'Best sellers',
    generating: 'Generating…', export_ready: 'Export ready',

    whatsapp: 'WhatsApp number', 
    back: 'Back',
    pending: 'Pending',
    confirmed: 'Confirmed', 

    settings_title: 'Settings', settings_sub: 'Rules, roles and money',
    roles_perms: 'Roles & permissions', exchange_rate: 'Exchange rate',
    loyalty_rules: 'Loyalty rules', reminders: 'Automatic reminders', branding: 'Branding',
    save_changes: 'Save changes', role_admin: 'Admin', role_manager: 'Manager',
    role_cashier: 'Cashier', role_warehouse: 'Warehouse', permission: 'Permission',
    points_per: 'Points per 1,000 SYP spent', point_value: 'Value of 1 point',
    rate_hint: '1 USD equals', shop_name: 'Shop name', accent_colour: 'Accent colour',

    next: 'Next', 
    of: 'of', close: 'Close', cancel: 'Cancel', save: 'Save',
    all_word: 'All', none: 'None', yes: 'Yes', no: 'No', remove: 'Remove',
    print_job: 'Print job',
    size_gap: 'Size gap', gap_only: 'Size gaps', pieces: 'pcs', all_new: 'all new',

    /* The loading screen (js/splash.js): one name per part of the system,
       and the noun that follows the number it landed with. */
    sp_loading: 'Loading the shop', sp_ready: 'Ready',
    sp_m_config: 'Shop settings', sp_m_catalogue: 'Catalogue', sp_m_customers: 'Customers',
    sp_m_sales: 'Sales', sp_m_movements: 'Stock', sp_m_partner: 'Print jobs',
    sp_m_purchase: 'Purchase orders', sp_m_money: 'The drawer', sp_m_counts: 'Stock counts',
    sp_m_suppliers: 'Suppliers', sp_m_employees: 'Staff', sp_m_alerts: 'Alerts',
    sp_m_dashboard: 'Dashboard', sp_m_reports: 'Reports', sp_m_labelTemplates: 'Labels',
    sp_m_mirror: 'Cloud copy',
    sp_u_warehouse: 'warehouses', sp_u_warehouse_1: 'warehouse',
    sp_u_product: 'products', sp_u_product_1: 'product',
    sp_u_customer: 'customers', sp_u_customer_1: 'customer',
    sp_u_invoice: 'invoices', sp_u_invoice_1: 'invoice',
    sp_u_movement: 'stock movements', sp_u_movement_1: 'stock movement',
    sp_u_job: 'jobs', sp_u_job_1: 'job',
    sp_u_order: 'orders', sp_u_order_1: 'order',
    sp_u_count: 'counts', sp_u_count_1: 'count',
    sp_u_supplier: 'suppliers', sp_u_supplier_1: 'supplier',
    sp_u_employee: 'people', sp_u_employee_1: 'person',
    sp_u_alert: 'alerts', sp_u_alert_1: 'alert',
    sp_u_template: 'templates', sp_u_template_1: 'template',
    sp_u_row_waiting: 'rows waiting', sp_u_row_waiting_1: 'row waiting',
    sp_u_row_pulled: 'rows pulled', sp_u_row_pulled_1: 'row pulled',
    sp_shift_open: 'shift open', sp_shift_closed: 'no open shift',
    sp_computed: 'computed', sp_mirror_ok: 'in step',
    sp_mirror_off: 'not set up', sp_mirror_attention: 'needs attention'
  },

  ar: {
    nav_main: 'العمليات', nav_ops: 'الأعمال',
    nav_dashboard: 'لوحة التحكم', nav_pos: 'الكاشير', nav_products: 'المنتجات',
    nav_warehouse: 'المستودع', nav_customers: 'الزبائن', nav_print: 'أعمال الطباعة',
    nav_reports: 'التقارير', nav_settings: 'الإعدادات',
    tagline: 'أحذية وملابس ستريت وير', live: 'نظام OG', search_top: 'ابحث في المحل…', search_products: 'ابحث عن منتج…', search_customers: 'ابحث عن زبون…',

    admin: 'المدير',
    notifications: 'التنبيهات', view_all: 'عرض الكل', no_results: 'لا توجد نتائج مطابقة',
    nt_all_read: 'تم تعليم كل التنبيهات كمقروءة',

    /* --- الحساب --- */
    sign_out: 'تسجيل الخروج', signing_out: 'جارٍ الخروج…',
    change_pw: 'تغيير كلمة المرور', my_account: 'حسابي',
    pw_current: 'كلمة المرور الحالية', pw_new: 'كلمة المرور الجديدة', pw_again: 'أعد كتابتها',
    pw_changed: 'تم تغيير كلمة المرور', pw_reauth: 'سجّل الدخول من جديد بكلمة المرور الجديدة',
    pw_mismatch: 'كلمتا المرور غير متطابقتين',
    pw_must_change: 'تمت إعادة تعيين كلمة مرورك — يرجى تغييرها',
    role_delivery: 'التوصيل', role_partner: 'شريك الطباعة',
    /* --- جدول الصلاحيات --- */
    roles_editable: 'حدّد ما يستطيع كل دور فعله. يُحفظ فوراً.',
    perm_saved: 'تم تحديث الدور', perm_refused: 'رُفضت بعض التغييرات',
    pg_till: 'الكاشير', pg_stock: 'المخزون', pg_products: 'المنتجات',
    pg_customers: 'الزبائن', pg_money: 'المال', pg_print: 'الطباعة',
    pg_admin: 'الإدارة', pg_partner: 'يلا وير', pg_delivery: 'التوصيل',

    /* --- الشاشات الرئيسية حسب الدور --- */
    hi_morning: 'صباح الخير', hi_afternoon: 'مساء الخير', hi_evening: 'مساء الخير',
    my_sales_today: 'مبيعاتي اليوم', my_invoices: 'فواتيري',
    my_last_sales: 'آخر مبيعاتي', open_till: 'افتح الكاشير',
    nothing_sold_yet: 'لم تُسجَّل أي فاتورة اليوم', first_sale_hint: 'امسح باركود لتبدأ',
    low_on_shelf: 'قارب على النفاد في المعرض', low_on_shelf_sub: 'اطلب من المستودع',
    shelf_all_good: 'المعرض مجهّز بشكل جيد',
    back_title: 'المستودع', back_sub: 'ما وصل وما يحتاج نقلاً',
    arrived_today: 'وصل اليوم', to_move_out: 'بانتظار النقل إلى المعرض',
    empty_on_floor: 'مقاسات فارغة في المعرض', open_orders: 'طلبيات في الطريق',
    back_receive: 'إدخال بضاعة', back_count: 'ابدأ جرداً',
    back_nothing: 'لا شيء بالانتظار', back_nothing_sub: 'المعرض مجهّز من المستودع',

    /* --- التوصيل --- */
    nav_deliveries: 'التوصيل',
    dl_title: 'التوصيل', dl_sub: 'ما يخرج وما يعود',
    dl_my_runs: 'توصيلاتي اليوم', dl_my_runs_sub: 'اضغط على البطاقة عند الانتهاء',
    dl_waiting: 'للخروج', dl_out: 'في الطريق', dl_delivered: 'تم التسليم',
    dl_failed: 'لم يُسلَّم', dl_to_collect: 'يُحصَّل', dl_collected: 'حُصِّل',
    dl_nothing_owed: 'مدفوع مسبقاً', dl_call: 'اتصال', dl_map: 'الخريطة',
    dl_take: 'خذها معك', dl_done: 'سُلِّمت', dl_fail: 'تعذّر التسليم',
    dl_why: 'ماذا حصل؟', dl_why_ph: 'لا أحد في البيت، عنوان خاطئ، رفض الاستلام…',
    dl_none: 'لا يوجد ما يُخرج', dl_none_sub: 'أنهيت كل شيء',
    dl_none_board: 'لا توجد توصيلات بعد',
    dl_none_board_sub: 'أرسل واحدة من فاتورة في الكاشير أو التقارير',
    dl_unassigned: 'لم يُحدَّد', dl_assign: 'إرسال للتوصيل',
    dl_address: 'العنوان', dl_address_ph: 'الشارع والبناء والطابق — ما يكفي للوصول للباب',
    dl_phone: 'الهاتف',
    dl_marked: 'تم التحديث', dl_sent: 'خرجت للتوصيل',
    dl_no_address: 'اكتب العنوان أولاً', dl_pick_driver: 'اختر السائق',
    retry: 'حاول مرة أخرى', loading: 'جارٍ التحميل…',

    /* --- الفاتورة المطبوعة --- */
    rc_title: 'الفاتورة', rc_full_page: 'فاتورة بحجم كامل',
    rc_policy: 'الاستبدال خلال 48 ساعة مع هذه الفاتورة',
    rc_paper: 'ورق الفاتورة', rc_80: '٨٠ مم', rc_58: '٥٨ مم',
    rc_paper_hint: 'قياس الرول في طابعتك. ٨٠ مم هو القياس المعتاد.',

    rc2_invoice: 'رقم الفاتورة', rc2_datetime: 'التاريخ والوقت', rc2_cashier: 'الكاشير',
    rc2_customer: 'الزبون', rc2_phone: 'الهاتف', rc2_points_balance: 'رصيد النقاط',
    rc2_size: 'مقاس', rc2_line_discount: 'خصم',
    rc2_subtotal: 'المجموع الفرعي', rc2_discount: 'الخصم', rc2_points_used: 'نقاط مستخدمة',
    rc2_total: 'الإجمالي', rc2_payment: 'طريقة الدفع', rc2_to_collect: 'المبلغ المطلوب تحصيله',
    rc2_points_earned: 'نقاط مكتسبة', rc2_shop_copy: 'نسخة المحل',
    rc2_gift_copy: 'فاتورة هدية', rc2_exchange_before: 'الاستبدال قبل',
    gift_receipt: 'فاتورة هدية',
    gift_pick_title: 'فاتورة هدية',
    gift_pick_hint: 'حدّد ما سيوضع في كيس الهدية. القسيمة لا تُظهر أي أسعار — فقط ما هي القطعة، ومتى اشتُريت، وآخر موعد لاستبدالها.',
    gift_pick_none: 'حدّد قطعة واحدة على الأقل.',
    pos_gift: 'هذه الفاتورة تحتوي على هدية',
    rc2_scan_exchange: 'امسح للاستبدال',
    rc_approve_title: 'تأكيد الفاتورة',
    rc_approve_hint: 'هذا ما سيُطبع تماماً. راجعه ثم اطبع.',
    rc_approve_cancel: 'ليس الآن',
    print_receipt: 'طباعة الفاتورة', printing: 'جارٍ الطباعة',
    print_sent: 'أُرسلت إلى الطابعة', print_retry: 'أعد المحاولة من الفاتورة.',

    rc3_title: 'طابعة الفاتورة', rc3_sub: 'الفاتورة الحرارية ٨٠ مم التي يحملها الزبون',
    rc3_transport: 'الاتصال', rc3_transport_network: 'الشبكة', rc3_transport_usb: 'USB (هذا الجهاز)',
    rc3_printer_share: 'مشاركة الطابعة', rc3_printer_share_hint: 'اسم مشاركة الطابعة في ويندوز كما تم إعدادها — راجع خطوات الإعداد لإضافتها كـ Generic / Text Only ومشاركتها.',
    rc3_instagram: 'رابط انستغرام', rc3_telegram: 'رابط تيليغرام', rc3_maps_url: 'رابط الموقع',
    rc3_host: 'عنوان الطابعة (IP على الشبكة)', rc3_port: 'المنفذ', rc3_branch: 'اسم الفرع',
    rc3_auto_print: 'طباعة تلقائية عند البيع', rc3_auto_print_hint: 'أوقفها في عرض تجريبي للعميل — طابعة تعمل خلال اجتماع مظهر غير مناسب.',
    rc3_confirm_print: 'التأكيد قبل الطباعة',
    rc3_confirm_print_hint: 'إظهار الفاتورة النهائية وطباعتها فقط عند الضغط على «طباعة». عند الإيقاف تُرسل مباشرة إلى الطابعة — أسرع عند الازدحام، لكن لا يراها أحد قبل الزبون.',
    rc3_copies: 'عدد النسخ', rc3_cut_mode: 'القص', rc3_cut_partial: 'جزئي', rc3_cut_full: 'كامل',
    rc3_ink: 'قتامة الطباعة',
    rc3_ink_normal: 'عادي', rc3_ink_dark: 'غامق', rc3_ink_darker: 'أغمق',
    rc3_ink_hint: 'مدى قوة حرق الطابعة لكل نقطة. ارفعها إذا خرجت السطور الصغيرة باهتة أو متقطّعة، وأنقصها إذا امتلأت الحروف وصارت كتلة سوداء.',
    rc3_gift_window: 'مدة استبدال الهدية (أيام)',
    rc3_gift_window_hint: 'أطول من المدة العادية عن قصد — الهدية تُشترى قبل أن تُقدَّم. تطبع قسيمة الهدية آخر يوم يمكن الاستبدال فيه.',
    rc3_gift_policy_ar: 'نص قسيمة الهدية (عربي)',
    rc3_gift_policy_en: 'نص قسيمة الهدية (إنجليزي)',
    rc3_show_barcode: 'الباركود', rc3_show_loyalty: 'سطور نقاط الولاء',
    rc3_show_barcode_hint: 'رقم الفاتورة كباركود، ليُمسح عند الإرجاع بدل كتابته باليد.',
    rc3_show_loyalty_hint: 'النقاط المكتسبة والرصيد، تُطبع تحت المجموع لزبائن برنامج الولاء.',
    rc3_footer_ar: 'التذييل (عربي)', rc3_footer_en: 'التذييل (إنجليزي)',
    rc3_policy_ar: 'سياسة الاستبدال (عربي)', rc3_policy_en: 'سياسة الاستبدال (إنجليزي)',
    rc3_save: 'حفظ إعدادات الفاتورة', rc3_saved: 'تم حفظ إعدادات الفاتورة',

    rc3_g_printer: 'أي طابعة',
    rc3_g_head: 'ما يُطبع في الأعلى',
    rc3_g_head_note: 'ترويسة كل فاتورة. عنوان المحل لا يُطبع — الزبون واقف في المحل أصلاً.',
    rc3_g_how: 'كيف تُطبع',
    rc3_g_how_note: 'متى تخرج الفاتورة، وكم نسخة، وبأي قتامة تحرقها الرأس.',
    rc3_g_slip: 'ما يظهر على الفاتورة',
    rc3_g_slip_note: 'النصّان يُطبعان بلغتين: العربي فوق الإنجليزي، بالترتيب الذي تُقرأ به الورقة.',
    rc3_g_gift: 'قسيمة الهدية',

    lbl_title: 'ملصقات حرارية',
    lbl_print_now: 'اطبع الآن', lbl_preview_title: 'معاينة الملصق',
    lbl_queued: 'تمت إضافة {n} ملصق إلى {station}',
    lb_sizes: '{n} قياسات', lb_sel: '{n} محدد', lb_print_all: 'طباعة كل القياسات',
    lbl_pick_station: 'اختر محطة أولاً',
    lbl_calibrate_sent: 'تم إرسال أمر المعايرة',
    lbl_fallback: 'يتحول إلى Code128 — العرض غير كافٍ لـ EAN-13',
    lbl_station: 'المحطة', lbl_preset: 'المقاس', lbl_qty: 'العدد',
    lbl_batch_total: 'ملصق بالمجموع',
    lbl_thermal_section: 'ملصقات حرارية (XP-235B)',
    lbl_thermal_sub: 'ملصق الباركود على الصندوق، يُطبع عبر USB',
    lbl_queue_title: 'طابور الطباعة', lbl_queue_empty: 'لا يوجد شيء في الطابور',
    lbl_cancel: 'إلغاء',
    lbl_unknown_code: 'هذا الرمز لا يطابق أي منتج',
    lbl_attach_code: 'اربط هذا الرمز بمنتج',
    lbl_attach_search: 'ابحث عن منتج…', lbl_attach_save: 'حفظ',
    lbl_save: 'حفظ إعدادات الملصق', lbl_saved: 'تم حفظ إعدادات الملصق',
    lbl_host: 'عنوان الطابعة (شبكة، لوضع النقل المباشر فقط)',
    lbl_transport: 'طريقة النقل', lbl_transport_agent: 'وكيل (لابتوب USB)',
    lbl_transport_tcp: 'مباشر (شبكة)', lbl_density: 'الكثافة',
    lbl_gap: 'الفجوة بين الملصقات (مم)',

    /* --- ملصقات ٦٠ × ٤٠ مم (js/labels60.js) — البكرة الموجودة فعلياً --- */
    l60_shelf_title: 'ملصقات الرفوف',
    l60_room: 'الغرفة', l60_from: 'من', l60_to: 'إلى', l60_all: 'الكل',
    l60_print: 'اطبع {n}', l60_true_size: '{n} ملصق، بالحجم الحقيقي — 60 × 40 مم',
    l60_per_pair: 'ملصق لكل زوج في المخزون',
    l60_no_shelves: 'لم تُرتّب أي رفوف في هذا المستودع بعد.',
    l60_wh_unnumbered: 'هذا المستودع بلا رقم للملصقات، فلا يمكن بناء باركود الرف.',
    l60_no_wh_code: 'لا رقم مستودع', l60_no_barcode: 'تعذّر ترميز الرمز',
    l60_not_logged: 'تمت الطباعة، لكن لم تُسجّل في سجل الملصقات',

    /* --- خريطة المستودع (js/shelfmap.js) --- */
    sm_room: 'الغرفة',
    sm_edit: 'تعديل التخطيط', sm_done: 'تمّ',
    sm_no_rooms: 'لم تُرتَّب أي غرف بعد.',
    sm_reset_view: 'إعادة ضبط العرض',
    sm_room_loading: 'جارٍ تحميل الغرفة…',
    sm_room_hint: 'اسحب للتجوّل · مرّر للتقريب · انقر رفاً',
    sm_room_edit_hint: 'اسحب رفاً إلى جدار لنقله · اسحب جداراً لتغيير قياس الغرفة — الرفوف التي لا تتّسع تُضيَّق خاناتها',
    sm_drag_span_cm: '{a}–{b} سم',
    sm_drag_slid: 'أُزيح ليتّسع',
    sm_drag_corner: 'أُبعد عن الزاوية',
    sm_drag_floor: 'على الأرض، بلا جدار',
    sm_drag_short_cm: 'المتبقّي على هذا الجدار {n} سم وعرض الرفّ {w} سم',
    sm_drag_full: 'الرفّ {k} موجود هناك',
    sm_cm: 'سم',
    /* the camera */
    sm_cam: 'الكاميرا', sm_cam_orbit: 'دوران', sm_cam_walk: 'مشي',
    sm_fit_view: 'الكلّ', sm_top_view: 'من فوق', sm_front_view: 'من الأمام',
    sm_fullscreen: 'ملء الشاشة', sm_exit_fullscreen: 'إنهاء ملء الشاشة',
    sm_room_is_full: 'الغرفة تملأ الشاشة',
    sm_walk_hint: 'W A S D أو الأسهم للمشي · اسحب للنظر حولك · انقر رفاً',
    sm_walk_hint_touch: 'استخدم الأزرار للمشي والنظر · المس رفاً',
    sm_pad_fwd: 'إلى الأمام', sm_pad_back: 'إلى الخلف', sm_pad_left: 'يساراً', sm_pad_right: 'يميناً',
    sm_pad_look: 'انظر', sm_pad_toggle: 'أزرار المشي',
    sm_quality: 'الجودة', sm_quality_auto: 'تلقائي', sm_quality_high: 'عالية', sm_quality_low: 'منخفضة',
    sm_quality_note: 'الظلال تتغيّر الآن؛ تنعيم الحواف عند فتح الخريطة في المرّة القادمة.',
    sm_quality_dropped: 'هذا الجهاز يرسم الغرفة ببطء، فأُطفئت الظلال عليه من الآن.',
    /* rack size */
    sm_wall_cm: 'الموضع على الجدار (سم من اليسار)',
    sm_free: 'حر',
    sm_free_x: 'من الجدار الأيسر (سم، إلى منتصفه)',
    sm_free_y: 'من الجدار الأمامي (سم، إلى منتصفه)',
    sm_rot: 'الاتجاه', sm_rotate: 'تدوير ربع دورة',
    sm_free_hint: 'يقف على الأرض ويحتاج ممرّاً بعرض {n} سم إلى كل جدار ورفّ حوله.',
    sm_the_wall_n: 'الجدار الخلفي', sm_the_wall_e: 'الجدار الأيمن', sm_the_wall_s: 'الجدار الأمامي', sm_the_wall_w: 'الجدار الأيسر',
    sm_drag_free: 'حر، مدار {r}° · {g} سم إلى {n}',
    sm_drag_rotate: 'اضغط R للتدوير',
    sm_drag_through: 'يخترق {w}',
    sm_drag_overlap: 'يدخل في {n}',
    sm_err_overlap: 'الرفّ {k} يتداخل مع {n}.',
    sm_err_aisle: 'فقط {g} سم إلى {n} — يلزم {m} سم.',
    sm_err_through: 'الرفّ {k} سيخترق {w}.',
    sm_err_free_unmeasured: 'لا يقف الرفّ حرّاً إلا في غرفة مقاسة — أدخل عرض الغرفة وعمقها أولاً.',
    sm_err_rotation: 'يدور الرفّ بربع دورة: 0° أو 90° أو 180° أو 270°.',
    sm_err_placement: 'الرفّ يقف على جدار أو حرّاً على الأرض — لا الاثنين معاً.',
    sm_live_changed: 'تغيّر ترتيب الغرفة على شاشة أخرى.',
    sm_live_take: 'اعرض الترتيب الجديد',
    sm_add_rack: 'إضافة رفّ', sm_add_wall: 'على جدار',
    sm_add_place: 'ضعه في الغرفة',
    sm_add_hint: 'حرّكه إلى مكانه وانقر لتضعه. Esc يلغي — لا يُحفظ شيء قبل ذلك.',
    sm_add_name: 'رفّ {k}',
    sm_add_no_letter: 'كل الحروف من A إلى Z مستخدمة لرفوف في هذا المستودع.',
    sm_grow_bays: '{n} خانات', sm_grow_levels: '{n} طبقات',
    sm_grow_near: '{g} سم إلى {n}',
    sm_grow_ceiling: 'ارتفاعه {h} سم — والغرفة {r} سم',
    sm_grow_wall: 'يتجاوز نهاية {w}',
    sm_err_too_tall: 'الرفّ {k} سيصبح ارتفاعه {h} سم والغرفة {r} سم — {m} طبقات على الأكثر.',
    sm_err_room_too_low: 'الرفّ {k} ارتفاعه {h} سم — يجب ألا تقلّ الغرفة عن ذلك.',
    sm_err_bay_occupied: 'خانة {b} فيها {n} قطعة — فضّيها أولاً.',
    sm_err_level_occupied: 'طبقة {l} فيها {n} قطعة — فضّيها أولاً.',
    sm_rack_size: 'قياس الرفّ',
    sm_rack_bay: 'عرض الخانة (سم)', sm_rack_level: 'ارتفاع الطبقة (سم)', sm_rack_depth: 'العمق (سم)',
    sm_rack_size_hint: 'إن تُرك فارغاً فهو رفّ المحل القياسي: خانة {b} سم، طبقة {l} سم، عمق {d} سم.',
    sm_rack_size_hint_room: 'إن تُرك فارغاً أخذ قياس رفوف هذه الغرفة: خانة {b} سم، طبقة {l} سم، عمق {d} سم.',
    sm_shrink_preview: '{k} ← خانات {n} سم',
    sm_shrunk: 'ضُيِّقت خانات الرفوف لتتّسع الغرفة',
    /* place my racks */
    sm_fill_racks: 'ضع رفوفي',
    sm_fill_title: 'وضع الرفوف على الجدران',
    sm_fill_line: 'على {w} عند {n} سم',
    sm_fill_go: 'ضعها',
    sm_fill_none_left: 'كلّ رفّ فيه طبقات موجود في غرفة.',
    sm_fill_partial: 'لا مكان على الجدران لـ: {k}. تبقى غير موضوعة.',
    /* the layout refusals */
    sm_err_wall_overlap: 'الرفّ {k} موجود على هذا الجدار عند {a}–{b} سم.',
    sm_err_wall_corner: 'الرفّ {k} يشغل هذه الزاوية.',
    sm_err_wall_short: 'عرض الرفّ {k} هو {w} سم والمتبقّي على الجدار هناك {n} سم.',
    sm_err_room_too_small: 'الرفّ {k} لن يتّسع. هذه الغرفة تحتاج على الأقل {w} × {d} سم.',
    sm_err_room_too_small_short: '{k} يحتاج {n} سم',
    sm_use_min: 'استخدم الحدّ الأدنى',
    sm_room_max: 'على الأكثر {n} م للضلع.',
    sm_drag_nofit: 'لا مجال على هذا الجدار',
    sm_drag_out: 'خارج الغرفة',
    sm_overflow: '{k} يتجاوز {w} بمقدار {m} سم',
    sm_room_size: '{w} × {d} م',
    sm_other_rooms: 'في غرف المستودع الأخرى',
    sm_no_webgl: 'تعذّر رسم الغرفة ثلاثية الأبعاد على هذا الجهاز — عرض الخريطة المسطّحة بدلاً منها.',
    sm_gl_lost: 'توقّف العرض ثلاثي الأبعاد على هذا الجهاز — عدنا إلى الخريطة المسطّحة.',
    sm_colour_by_type: 'تلوين حسب النوع',
    sm_view: 'العرض', sm_view_2d: 'المخطّط', sm_view_room: 'غرفة ثلاثية',
    sm_room_off_motion: 'الغرفة تحتاج حركة، وهذا الجهاز طلب تقليلها.',
    sm_remove_level: 'احذف الطبقة {x}', sm_remove_bay: 'احذف الخانة {x}',
    sm_flat_hint: 'انقر رفاً في المخطّط لفتحه · أشر إلى خانة لترى ما عليها',
    sm_rack_empty: 'لا رفوف على هذا الرفّ بعد.',
    sm_design: 'تصميم الغرفة',
    sm_racks_here: 'الرفوف في هذه الغرفة', sm_no_racks: 'لا رفوف في هذه الغرفة بعد',
    sm_new_rack: 'رفّ جديد', sm_rack_cfg: 'إعدادات الرفّ', sm_rack_key: 'حرف الرفّ',
    sm_rack_name: 'اسم الرفّ', sm_delete_rack: 'حذف الرفّ',
    sm_room_settings: 'إعدادات الغرفة', sm_not_placed: 'لم يُوضع في غرفة بعد',
    sm_place: 'وضع', sm_nowhere: '— غير موضوع —',
    sm_wall: 'الجدار', sm_wall_n: 'الجدار الخلفي', sm_wall_e: 'الجدار الأيمن', sm_wall_s: 'الجدار الأمامي', sm_wall_w: 'الجدار الأيسر',
    sm_levels: 'الطبقات', sm_bays: 'الخانات', sm_add_level: 'إضافة طبقة', sm_add_bay: 'إضافة خانة',
    sm_level_hint: 'A هي الطبقة العليا؛ الطبقة الجديدة تُضاف في الأسفل.',
    sm_room_w: 'العرض (سم)', sm_room_d: 'العمق (سم)', sm_room_h: 'الارتفاع (سم)',
    sm_measure_hint: 'اختياري. إن تُرك فارغاً تُقاس الغرفة على رفوفها وتُعلَّم "ليست بالمقياس".',
    sm_not_to_scale: 'ليست بالمقياس',
    sm_racks_first: 'أضف غرفة، ثم رفوفاً داخلها.',
    ty_sneakers: 'أحذية رياضية', ty_boots: 'بوط', ty_tshirts: 'تيشيرتات', ty_jeans: 'جينز',
    ty_jerseys: 'جيرسي', ty_crocs: 'كروكس', ty_shirts: 'قمصان', ty_jackets: 'جاكيتات',
    sm_new_room: 'غرفة جديدة', sm_delete_room: 'احذف الغرفة',
    sm_room_name: 'اسم الغرفة، بكلماتك',
    sm_origin: 'الخانة 1 تبدأ من', sm_origin_left: 'اليسار', sm_origin_right: 'اليمين',
    sm_origin_hint: 'كما تقف على باب تلك الغرفة. الخريطة تطابق الغرفة، لا لغة القوائم.',
    sm_grid_setup: 'رتّب الشبكة',
    sm_capacity: 'السعة (صناديق)', sm_cap_hint: 'اتركها فارغة إن لم تكن معروفة',
    sm_capacity_n: 'يتّسع لـ {n}',
    sm_remove_n: 'سيُحذف {n} رف، منها {a} مخصّص. البضاعة تمنع الحذف.',
    sm_remove_go: 'احذف',
    sm_no_renumber: 'حذف صف أو خانة لا يعيد ترقيم الباقي أبداً — كل ملصق مطبوع يبقى صحيحاً.',
    sm_unassigned: 'غير مخصّص — يأخذ أول ما يوضع عليه',
    sm_unassigned_opt: '— غير مخصّص —',
    sm_whole_model: 'كل الموديل',
    sm_shelf_empty: 'لا شيء على هذا الرف.',
    sm_front_area: 'منطقة الاستلام',
    sm_bay_where: 'رف {r} · طبقة {l} · خانة {b}',
    sm_bay_total: '{n} قطعة',
    sm_bay_empty: 'لا شيء هنا بعد — امسح زوجاً لتضعه على {code}.',
    sm_bay_more: '+{n} مقاسات أخرى',
    sm_bay_back: 'اضغط Esc أو اضغطه مرة ثانية للرجوع',
    sm_code: 'الرمز', sm_delete: 'احذف الرف',
    sm_locked: 'عليه {n} قطعة — لا يمكن حذفه',
    sm_confirm_title: 'هذا الرف ليس فارغاً',
    sm_confirm_stock: 'عليه الآن {n} قطعة من {p}.',
    sm_confirm_code: 'تغيير الرمز يجعل ملصق هذا الرف المطبوع خاطئاً.',
    sm_stale: '{n} ملصق مطبوع سيصبح قديماً',
    sm_reprint: 'أعد طباعة الملصقات', sm_apply: 'طبّق رغم ذلك',
    sm_print_label: 'اطبع ملصق الرف',
    sm_target: 'التخزين على {code}',
    sm_scan_hint: 'امسح القطع التي ستضعها عليه',
    sm_no_home: 'هذا الموديل بلا رف بعد',
    sm_shelf_gone: 'لا رف بهذا الرمز على الخريطة',
    sm_unknown_code: 'ليس رمز رف ولا رمز منتج',
    sm_shelves_title: 'تخصيص الرفوف',
    sm_shelves_sub: 'أي موديل لكل رف. الخريطة نفسها في «خريطة الرفوف».',
    sm_open_map: 'افتح الخريطة',
    nav_shelfmap: 'خريطة الرفوف',
    sm_scan_ph: 'امسح رفاً ثم امسح الأزواج…',
    sm_no_target: 'لا رف محدّد',
    sm_no_target_hint: 'امسح ملصق رف لتبدأ التخزين، أو امسح زوجاً لتعرف مكانه',
    sm_target_set: 'صار الهدف',
    sm_refused: 'مرفوض',
    sm_last_scans: 'آخر المسحات',
    sm_undo_hint: 'اضغط للتراجع عن هذه المسحة',
    sm_clear_strip: 'امسح الشريط',
    sm_sound_on: 'الصوت يعمل', sm_sound_off: 'الصوت مطفأ',

    /* --- الحسومات --- */
    disc_capped: 'الحسم فوق {p}٪ يحتاج موافقة المدير',
    disc_max: 'أقصى حسم ممكن: {v}',
    wh_cost_later: 'المدير يضيف سعر التكلفة. اكتب سعر البيع فقط.',

    dash_title: 'لوحة التحكم', dash_sub: 'كل شيء عن المحل في شاشة واحدة',
    st_today: 'مبيعات اليوم', st_products: 'المنتجات',
    st_critical: 'أصناف بمخزون حرج', st_customers: 'الزبائن النشطون', st_print: 'طلبات طباعة معلّقة',
    vs_last_month: 'مقارنة بالشهر الماضي',
    in_catalogue: 'في الكتالوج',
    sales_6m: 'المبيعات — آخر ٦ أشهر', sales_by_type: 'المبيعات حسب نوع المنتج',
    needs_attention: 'تحتاج انتباهك', recent_sales: 'آخر المبيعات', fix: 'معالجة',

    invoice: 'الفاتورة', customer: 'الزبون', date: 'التاريخ', items: 'الأصناف', total: 'الإجمالي',
    payment: 'الدفع', cashier: 'الكاشير', actions: 'إجراءات', qty: 'الكمية', size: 'القياس',
    unit_price: 'سعر القطعة', line_total: 'الإجمالي', price: 'السعر', cost: 'التكلفة',
    margin: 'الربح %', stock: 'المخزون', type: 'النوع', product: 'المنتج', name: 'الاسم',
    phone: 'الهاتف', city: 'المدينة', status: 'الحالة', notes: 'ملاحظات', barcode: 'الباركود',
    shelf: 'الرف', sku: 'الرمز', brand: 'الماركة', made_in: 'بلد الصنع', colour: 'اللون',

    pos_title: 'الكاشير',
    scan_ph: 'امسح الباركود أو ابحث عن منتج…', scan_btn: 'مسح',
    cart: 'السلة', clear: 'إفراغ', empty_cart: 'السلة فارغة',
    empty_cart_sub: 'امسح باركود أو اضغط على منتج لبدء البيع',
    all_products: 'الكل', subtotal: 'المجموع', discount: 'الخصم', coupon: 'كود الخصم',
    apply: 'تطبيق', applied: 'مطبّق', points: 'نقطة', loyalty: 'الولاء',
    use_points: 'استخدم', payment_method: 'طريقة الدفع', add_print: 'أضف طلب طباعة لهذه الفاتورة',
    complete_sale: 'إتمام البيع', customer_ph: 'اختر زبوناً أو اكتب للبحث…', change_customer: 'تغيير',
    walk_in: 'زبون عابر', priority: 'الأولوية',
    deadline: 'موعد التسليم', normal: 'عادي', urgent: 'مستعجل', amount: 'مبلغ', percent: 'نسبة',
    in_stock: 'متوفر', pick_size: 'اختر القياس', out_of_stock: 'غير متوفر',
    sale_complete: 'تمّ البيع', points_earned: 'نقاط الولاء المكتسبة',
    new_sale: 'بيع جديد', print: 'طباعة', pdf: 'PDF', thank_you: 'شكراً لك — نراك قريباً',
    served_by: 'بواسطة', bill_to: 'الفاتورة إلى',

    products_title: 'المنتجات', products_sub: 'كل صنف وكل قياس ومخزون لحظي',
    health: 'حالة المخزون', visible: 'ظاهر بالموقع', healthy: 'جيد', low: 'منخفض',
    critical: 'حرج', out: 'منتهي', all_types: 'كل الأنواع', all_health: 'كل الحالات',
    per_size: 'المخزون حسب القياس', sales_trend: 'حركة المبيعات — آخر ٦ أشهر',
    size_gap_warn: 'فجوة قياسات — الكمية جيدة لكن القياسات المطلوبة صفر',
    total_stock: 'إجمالي المخزون', stock_value: 'قيمة المخزون بالتكلفة', last_sold: 'آخر بيع',
    ms_title: 'نقل بالمسح',
    ms_swap: 'تبديل المكانين',
    ms_scan_hint: 'امسح باركود، أو اكتب الرمز واضغط Enter',
    ms_camera: 'الكاميرا',
    ms_empty: 'لا شيء في القائمة بعد',
    ms_empty_sub: 'امسح كل قطعة تحملها. لا شيء يُنقل حتى تضغط الزر بالأسفل.',
    ms_available: 'المتوفر هناك', ms_moving: 'المنقول',
    ms_move_n: 'نقل {n} قطعة',
    ms_on_list: '{n} في القائمة',
    ms_none_at: 'لا يوجد شيء في {place}',
    ms_over: '{n} سطر يطلب أكثر مما في المكان \u2014 سيُنقل الموجود فقط.',
    ms_note: 'نُقل إلى المحل',
    ms_some_failed: 'تعذّر نقل {n}',
    wh_moves_note: 'كل حركة مسجّلة باسم المستخدم والتاريخ والرصيد بعدها. لا يمكن حذف سطر — فقط إضافة حركة تصحيح.',
    edit_product: 'تعديل المنتج', reorder: 'طلب توريد',
    bk_delete: 'حذف',
    bk_del_confirm: 'حذف {n} منتج نهائياً؟\n\n{names}\n\nلا يمكن التراجع. أي منتج تم بيعه أو نقله أو طلبه أو جرده سيبقى وسيُذكر اسمه — أرشِف تلك المنتجات.',
    bk_deleted: 'حُذف {n}',
    bk_del_kept: 'تعذّر حذف {n}',
    pe_note: 'القياسات والمخزون لا تُعدَّل هنا: أدخِل البضاعة وأخرِجها من المستودع، حتى يبقى كل تغيير في سجل الحركات.',
    name_required: 'الاسم مطلوب.', price_required: 'سعر البيع مطلوب.',
    pe_saved: 'تم الحفظ',

    warehouse_title: 'المستودع', warehouse_sub: 'لا تتحرك قطعة دون أثر',
    tab_add: 'إضافة منتج', tab_moves: 'حركات المخزون',

    wh_stock: 'المخزون حسب المكان', wh_all: 'كل الأماكن',
    wh_location: 'المكان', wh_sell_from: 'اسحب البضاعة من',
    wh_not_here: 'مو موجود بالمحل', wh_in_the_back: 'بالمستودع',
    wh_bring_out: 'جيب وحدة', wh_moved: 'تم النقل',
    wh_count_where: 'وين رح تعدّ؟',
    wh_transfer: 'نقل بضاعة', wh_move: 'انقل', wh_from: 'من', wh_to: 'إلى',
    wh_qty_to_move: 'كم قطعة', wh_move_done: 'انتقلت البضاعة',
    wh_suggest: 'طلّع هدول من المستودع',
    wh_suggest_sub: 'صفر بالمحل وموجودة بالمستودع',
    wh_here: 'هون',
    wh_pieces_here: 'قطعة هون', wh_value_here: 'القيمة بالتكلفة',
    wh_skus_here: 'قياس موجود هون',

    or_state: 'الطلب', or_draft: 'ما انبعت بعد', or_pending: 'بانتظار يلا وير',
    or_accepted: 'مقبول', or_declined: 'مرفوض',
    or_send: 'أرسل ليلا وير', or_send_again: 'أرسل مرة تانية',
    or_send_title: 'إرسال الطلب ليلا وير',
    or_send_hint: 'هيدا بالضبط يلي رح يشوفوه. فيهم يقبلوا أو يعتذروا.',
    or_sent_ok: 'انبعت الطلب — بانتظار موافقة يلا وير',
    or_cannot: 'ما فينا نبعت هالطلب',
    or_why_tbc: 'في قمصان لسا بلا أسماء. يلا وير ما بتقدر تطبع ظهر فاضي — أكّد الأسماء أول.',
    or_why_sent: 'هالطلب أصلاً عند يلا وير.',
    or_why_accepted: 'يلا وير قبلته من قبل.',
    or_wait_head: 'بانتظار رد يلا وير',
    or_wait_sub: 'انبعت وما إجا رد',
    or_declined_head: 'يلا وير اعتذرت عن هالطلب',
    or_promised: 'وعدوا فيه', or_requested: 'إنت طلبت',
    or_timeline: 'مسار الطلب',
    or_tl_created: 'إنشاء الطلب', or_tl_sent: 'انبعت ليلا وير',
    or_tl_accepted: 'يلا وير قبلت', or_tl_declined: 'يلا وير اعتذرت',
    or_by_og: 'OG', or_by_yalla: 'يلا وير',
    or_blocked_stage: 'يلا وير لسا ما قبلت هالطلب',
    or_wa: 'ابعته عالواتساب كمان',
    or_wa_title: 'إرسال الطلب عالواتساب',

    yl_new_orders: 'طلبات جديدة', yl_new_orders_sub: 'انبعتت من OG وناطرة ردّك',
    yl_accept: 'اقبل', yl_decline: 'اعتذر',
    yl_accept_title: 'قبول الطلب', yl_decline_title: 'الاعتذار عن الطلب',
    yl_promise: 'جاهز بتاريخ', yl_promise_hint: 'عهالتاريخ رح ينقاس التزامك، مو عتاريخ OG.',
    yl_decline_hint: 'خبّر OG ليش، بكلامك. رح يشوفوها فوراً.',
    yl_decline_ph: 'مثلاً: المكبس محجوز لتاريخ الأحد…',
    yl_accepted_ok: 'قبلت الطلب — صار عندك عاللوح',
    yl_declined_ok: 'اعتذرت — وانبلغت OG',
    yl_requested_by: 'OG طلبت',
    yl_sent_ago: 'انبعت',
    wh_empty_sizes: 'قياس فاضي هون', wh_none_here: 'ما في شي بهالمكان',
    wh_nothing_to_move: 'ما في شي لازم يطلع — المحل معبّى',
    wh_split_hint: 'المحل / المستودع',
    image: 'الصورة', product_name: 'اسم المنتج',
    nav_more: 'المزيد', language: 'اللغة',
    wa_open: 'افتح في واتساب', wa_opened: 'فُتح واتساب والرسالة جاهزة',
    wa_handoff: 'يفتح واتساب والرسالة مكتوبة. الإرسال يتم من هناك.',
    wa_bad_number: 'رقم الهاتف غير صالح',
    wa_day_title: 'أرسل ملخّص اليوم', wa_day_empty: 'لا مبيعات مسجّلة اليوم بعد',
    wa_send_day: 'أرسل اليوم',
    po_place: 'إرسال الطلبية', po_rate: 'يُباع', po_week: 'أسبوع', po_cover: 'يكفي',
    po_order: 'الطلب', po_no_sales: 'لا مبيعات',
    po_need_qty: 'أدخل كمية على مقاس واحد على الأقل',
    po_explain: 'الكميات مقترحة من المبيعات الفعلية آخر ٨ أسابيع، بما يكفي ٤ أسابيع قادمة. ' +
                'يمكنك تعديل أي رقم.',
    po_whatsapp: 'أرسل على واتساب', po_received_toast: 'أُدخلت إلى المخزون',
    po_title: 'طلبيات الشراء', po_draft: 'مسودة', po_sent: 'مُرسلة', po_received: 'مستلمة',
    tab_reorder: 'يستحق إعادة الطلب',
    po_open: 'طلبيات مفتوحة',
    po_receive: 'استلام البضاعة', po_none: 'لا طلبيات شراء بعد',
    po_none_sub: 'اطلب أي منتج منخفض وستظهر هنا',
    po_suggest: 'يستحق إعادة الطلب', po_suggest_sub: 'من سرعة البيع الفعلية، الأكثر إلحاحاً أولاً',
    sc_title: 'مسح', sc_looking: 'وجّه الكاميرا نحو الملصق',
    sc_found: 'تم العثور على', sc_found_title: 'نتيجة المسح', sc_again: 'امسح آخر',
    sc_photo: 'التقط صورة بدلاً من ذلك', sc_torch: 'إضاءة',
    sc_no_torch: 'لا إضاءة في هذه الكاميرا',
    sc_type_code: 'أو اكتب الباركود', sc_go: 'اذهب',
    sc_reading: 'جارٍ قراءة الصورة…', sc_nothing: 'لم يُعثر على رمز في الصورة',
    sc_unknown: 'لا يوجد تطابق في النظام',
    sc_denied: 'رُفض إذن الكاميرا — اسمح به أو استخدم صورة',
    sc_nodevice: 'لا توجد كاميرا على هذا الجهاز',
    sc_failed: 'تعذّر فتح الكاميرا',
    sc_no_camera: 'الكاميرا المباشرة تحتاج https',
    sc_no_camera_sub: 'عند الفتح من ملف يمنع المتصفح الكاميرا. التقط صورة — النتيجة نفسها.',
    sc_use_photo: 'التقط صورة للملصق',
    sc_this_size: 'المقاس الممسوح', sc_all_sizes: 'كل المقاسات', sc_scanned: 'ممسوح',
    sc_per_week: 'أسبوعياً',

    sc_sells: 'بينباع', sc_last_8w: 'خلال آخر ٨ أسابيع',
    sc_months: ' شهر', sc_over_a_year: 'أكتر من سنة',
    sc_cover: 'بيكفّي حوالي', sc_at_this_rate: 'عهالسرعة',
    sc_not_moving: 'ما عم ينباع', sc_never_sold: 'ما انباع بهالقياس أبداً',
    sc_sourcing: 'من وين بيجي', sc_last_delivery: 'آخر توريد',
    sc_no_delivery: 'ما في شي مسجّل بعد',
    sc_recent_moves: 'آخر الحركات', sc_no_moves: 'ما تحرّك شي على هالقياس بعد.',
    sc_what_now: 'شو رح تعمل فيها؟',
    sc_check_in: 'دخّلها عالمخزون', sc_check_out: 'طلّعها من المخزون',
    sc_sell: 'بيعها',
    sc_enter_hint: 'اضغط Enter لتبيعها فوراً — خلّي القارئ بإيدك.',
    sc_checked_in: 'دخلت عالمخزون', sc_checked_out: 'طلعت من المخزون',
    sc_in_note: 'إدخال بالمسح', sc_out_note: 'إخراج بالمسح',
    sc_cannot_out: 'ما في شي تطلّعه', sc_only_had: 'كان في بس',

    hw_title: 'القارئ والطابعة', hw_sub: 'الأجهزة، وكيف تتأكد إنها شغّالة',
    hw_scanner: 'قارئ الباركود',
    hw_scanner_note: 'القارئ بالكبل أو بالدونغل أو بالبلوتوث كلهم بيكتبوا متل الكيبورد، فكلهم بيشتغلوا بلا إعداد. امسح بالمربّع تحت لتتأكد.',
    hw_test: 'امسح أي باركود هون للتجربة', hw_waiting: 'بانتظار مسح…',
    hw_last: 'آخر مسح', hw_gap: 'السرعة', hw_accepted: 'انعرف كمسح',
    hw_rejected: 'انقرأ ككتابة، مو مسح',
    hw_prefix: 'حرف البداية', hw_prefix_note: 'اختياري. إذا برمجت القارئ يبعت حرف قبل الكود، بيصير التعرّف أكيد بدل ما يكون على أساس السرعة.',
    hw_threshold: 'حدّ السرعة', hw_threshold_note: 'القارئ بيكتب أسرع من هالعدد بالميلي ثانية للحرف. زيّده إذا صار يفوّت مسحات حقيقية.',
    hw_printer: 'طابعة الملصقات',
    hw_printer_note: 'خيار «هذا الجهاز» بيحتاج طابعة الملصقات مركّبة بويندوز كطابعة؛ خيار الطابور بيحتاج وكيل الطباعة أو عنوان الطابعة على الشبكة (قسم الملصقات تحت). اطبع ملصق تجربة، وبعدين المسطرة.',
    hw_mode: 'الورق', hw_roll: 'رول ملصقات', hw_sheet: 'ورقة ستيكر A4',
    hw_test_label: 'اطبع ملصق تجربة', hw_calibrate: 'اطبع مسطرة',
    hw_calibrate_note: 'المساطر المطبوعة لازم تقيس ١٠ ملم بالضبط. إذا ما قاست، الطابعة عم تكبّر أو تصغّر والقياس بالتعريف مو مطابق للرول.',
    hw_sym_note: 'الخطوط بتحمل رمز الملصق الخاص بالمحل (أو EAN-13 إذا كان الملصق عريض كفاية) — نفس الشي بكل شاشة، ومسح أي واحد منهم بيلاقي المقاس.',
    hw_camera_gap: 'عالموبايل يلي ما بيدعم الباركود أصلاً، الكاميرا بتقرأ EAN-13 بس مو كود ١٢٨. قارئ الباركود ما بيتأثر — بيفكّ الترميز عالجهاز.',
    sc_running_out: 'ينفد خلال {d} يوم تقريباً',
    sizes: 'المقاسات',
    st_count: 'جرد المخزون', st_intro_sub: 'امشِ على الرف، امسح، قارن',
    st_ready: 'جاهز للجرد',
    st_ready_sub: 'امسح أو اكتب ما هو موجود فعلاً على الرف. لا شيء يتغيّر حتى الترحيل — ' +
                  'تتم المقارنة مع النظام أولاً وترى كل فرق قبل تطبيقه.',
    st_started: 'بدأ', st_progress: 'تم جرده', st_variance: 'الفروقات',
    st_sizes_differ: 'مقاساً غير مطابق', st_net_pieces: 'صافي القطع', st_vs_system: 'مقابل النظام',
    st_value: 'القيمة', st_at_cost: 'بسعر التكلفة',
    st_system: 'النظام', st_counted: 'المجرود', st_diff: 'الفرق',
    st_f_all: 'الكل', st_f_variance: 'الفروقات فقط', st_f_counted: 'مجرود', st_f_uncounted: 'غير مجرود',
    st_find: 'ابحث عن منتج أو رمز أو باركود…',
    st_now_counted: 'العدد:', st_showing: 'يعرض {n} من {t} — استخدم البحث للتضييق',
    st_post: 'ترحيل الجرد', st_post_q: 'تعديل {n} مقاساً؟',
    st_post_sub: '{p} قطعة · {v} بالتكلفة',
    st_post_note: 'تُعدَّل المقاسات التي جردتها فقط. ما تُرك فارغاً لا يُمسّ، وكل تعديل يُسجَّل في حركات المخزون.',
    st_posted: 'رُحّل — {n} مقاساً معدّلاً، {p} قطعة',
    st_adjust_note: 'جرد مخزون', st_last: 'آخر جرد', st_adjusted: 'مقاسات معدّلة',
    st_discard_title: 'إلغاء هذا الجرد؟',
    st_discard_body: 'جردت {n} مقاساً. لم يُطبَّق شيء على المخزون بعد، والإغلاق الآن يفقد الجرد.',
    st_discard: 'ألغِه',
    dup_title: 'قد يكون هذا المنتج موجوداً أصلاً',
    dup_head: 'يوجد منتج مشابه جداً',
    dup_sub: 'إضافته مرتين تقسّم المخزون بين منتجين، ولا يكون أي عدد منهما صحيحاً.',
    dup_match: 'التطابق', dup_use: 'افتح هذا بدالو',
    dup_anyway: 'إنه منتج مختلف — أضفه',
    dup_note: 'أنت تضيف «{n}». إن كان لوناً أو موديلاً مختلفاً فعلاً، تابع.',
    mo_title: 'الحركة', mo_animations: 'الحركات والانتقالات',
    mo_hint: 'أطفئها إذا بدا التطبيق بطيئاً على جهاز عرض أو هاتف قديم.',
    mo_on: 'الحركات مفعّلة', mo_off: 'الحركات متوقفة',
    cp_title: 'بحث سريع', cp_placeholder: 'ابحث أو اكتب أمراً…',
    cp_new_sale: 'بيع جديد', cp_add_product: 'إضافة منتج',
    cp_scan_hint: 'الكاميرا', cp_go_to: 'اذهب إلى',
    cp_toggle_lang: 'تبديل اللغة', cp_toggle_curr: 'تبديل العملة',
    cp_cmd: 'أمر', cp_product: 'منتج', cp_customer: 'زبون',
    cp_invoice: 'فاتورة', cp_job: 'طلب طباعة',
    nav_money: 'المال', mn_title: 'المال', mn_sub: 'الصندوق، المصاريف، ومن له عليك',
    mn_shift: 'الوردية', mn_expenses: 'المصاريف', mn_debt: 'دفتر الدين',
    mn_open_shift: 'افتح وردية', mn_close_shift: 'إغلاق الوردية',
    mn_no_shift: 'لا توجد وردية مفتوحة',
    mn_no_shift_sub: 'افتح وردية بالمبلغ الموجود في الصندوق. كل بيع يُربط بها، وعند الإغلاق ' +
                     'يخبرك النظام بما يجب أن يكون في الصندوق.',
    mn_cashier: 'الكاشير', mn_float: 'رصيد الافتتاح',
    mn_float_hint: 'النقد الموجود في الصندوق قبل أول عملية بيع.',
    mn_shift_open: 'وردية مفتوحة', mn_open_since: 'مفتوحة منذ',
    mn_sales: 'مبيعات الوردية', mn_in_drawer: 'يجب أن يكون في الصندوق',
    mn_expected_now: 'الآن', mn_not_drawer: 'ليس في الصندوق',
    mn_settles_later: 'شام كاش، بطاقة — تُحصَّل لاحقاً',
    mn_drawer_now: 'الصندوق', mn_by_method: 'طريقة الدفع',
    mn_cash_sales: 'مبيعات نقدية', mn_debt_settled: 'ديون محصّلة', mn_cash_out: 'مدفوع نقداً',
    mn_expected: 'المتوقع', mn_counted: 'المعدود', mn_difference: 'الفرق',
    mn_count_now: 'عُدّ الصندوق واكتب المجموع',
    mn_close_hint: 'عُدّ النقود قبل النظر إلى الرقم المتوقع — المطابقة على رقم تعرفه مسبقاً ليست عدّاً.',
    mn_count_needed: 'اكتب ما عددته', mn_closed: 'أُغلقت',
    mn_balanced: 'مطابق تماماً', mn_short: 'ناقص', mn_over: 'زائد',
    mn_last_shift: 'آخر وردية', mn_in_box: 'في الصندوق', mn_to_account: 'إلى حساب',
    mn_owed: 'لك عند الزبون',
    mn_no_sales_yet: 'لا مبيعات في هذه الوردية بعد', mn_no_sales_sub: 'ستظهر هنا فور حدوثها',
    mn_add: 'إضافة', mn_add_expense: 'إضافة مصروف', mn_amount: 'المبلغ',
    mn_amount_needed: 'أدخل مبلغاً', mn_where: 'أين تذهب النقود',
    mn_category: 'التصنيف',
    mn_exp_month: 'مصاريف الشهر',
    mn_net: 'الربح الحقيقي',
    mn_c_rent: 'الإيجار', mn_c_generator: 'المولّدة / الوقود', mn_c_salaries: 'الرواتب',
    mn_c_transport: 'النقل', mn_c_packaging: 'التغليف', mn_c_supplier: 'المورّد',
    mn_c_other: 'أخرى',
    mn_owed_total: 'لك عند الزبائن', mn_people: 'فاتورة غير مدفوعة', mn_over_30: 'أقدم من ٣٠ يوماً',
    mn_chase_these: 'تستحق المتابعة', mn_oldest: 'الأقدم', mn_still_owed: 'المتبقي',
    mn_age: 'العمر', mn_settle: 'تحصيل', mn_remind: 'تذكير',
    mn_part_paid: 'دفعة جزئية، تبقّى', mn_cleared: 'سُدِّد بالكامل',
    mn_settle_hint: 'النقد المحصَّل أثناء وردية مفتوحة يدخل صندوقها.',
    mn_no_debt: 'لا أحد مدين لك', mn_no_debt_sub: 'مبيعات الدين تظهر هنا حتى تُسدَّد',
    up_pick: 'أضف صورة', up_hint: 'اضغط · اسحب · الصق',
    up_swap: 'تغيير الصورة',
    up_ok: 'تمت إضافة الصورة',
    img_change: 'تغيير الصورة', img_add: 'إضافة صورة', img_remove: 'إزالة', img_removed: 'أُزيلت الصورة',
    img_open: 'فتح الصورة', img_uploading: 'جاري إرسال الصورة إلى السحابة…',
    img_saved: 'حُفظت الصورة — وستظهر في كل مكان يظهر فيه هذا المنتج.',
    img_fail: 'حُفظ المنتج، لكن الصورة لم تصل إلى السحابة. أضفها لاحقاً من نافذة المنتج.',
    img_stale_server: 'حُفظ المنتج. الخادم يشغّل نسخة أقدم من الكود ولا يعرف بعد كيف يخزّن الصور — اضغط Hard refresh في OG System، ثم أضف الصورة من نافذة المنتج.',
    up_err_none: 'لم يتم اختيار أي ملف',
    up_err_type: 'هذا الملف ليس صورة',
    up_err_size: 'الصورة كبيرة جداً — الحد ١٢ ميغابايت',
    up_err_read: 'تعذّرت قراءة الملف',
    up_err_decode: 'الصورة تالفة أو بصيغة لا يدعمها المتصفح',
    cost_price: 'سعر التكلفة', selling_price: 'سعر البيع', shelf_box: 'الرف / الصندوق',
    /* A noun phrase, like the labels around it — "تُستلم في" reads as a
       sentence fragment sitting above a box. */
    wh_intake: 'مكان الاستلام',
    shelf_none: '— لم يُرتَّب بعد —',
    shelf_no_rooms: 'لا رفوف في هذا المكان بعد',
    wh_shelf_partial: 'لم تُوضع {n} من {m} مقاسات على الرف.',
    size_matrix: 'الكمية لكل قياس', matrix_hint: 'أدخل الكمية لكل قياس — تصدر الرموز عند الحفظ',
    print_labels: 'طباعة ملصقات الباركود', save_product: 'حفظ المنتج في المستودع',
    movement: 'الحركة', user: 'المستخدم',
    balance: 'الرصيد بعدها', received: 'وارد', sold: 'مبيع', damaged: 'تالف',
    returned: 'مرتجع', transfer: 'نقل',
    total_pieces: 'إجمالي القطع', total_cost: 'إجمالي التكلفة', expected_revenue: 'الإيراد المتوقع',

    customers_title: 'الزبائن', customers_sub: 'من يشتري وماذا يشتري ومتى توقّف',
    cu_new: 'زبون جديد', cu_name_ph: 'الاسم الكامل',
    cu_more: 'و{n} غيرهم — تابع الكتابة لتضييق النتائج',
    cu_new_note: 'ثلاث خانات وبس. النقاط تبدأ من صفر وتزيد من أول عملية بيع.',
    cu_exists: 'موجود في القائمة',
    no_access: 'حسابك ما عندو صلاحية لهالشي',
    tier: 'الفئة', total_spent: 'إجمالي الإنفاق', last_purchase: 'آخر شراء',
    send_whatsapp: 'إرسال واتساب', purchase_history: 'سجل المشتريات',
    points_timeline: 'سجل نقاط الولاء', preferred_sizes: 'القياسات المفضلة',
    source: 'المصدر', online: 'أونلاين', in_store: 'من المحل', gold: 'ذهبي', silver: 'فضي',
    bronze: 'برونزي', all_customers: 'كل الزبائن',
    gold_only: 'الذهبيون فقط', whatsapp_msg: 'رسالة واتساب', send: 'إرسال',
    days_ago: 'يوم مضى', today_word: 'اليوم', yesterday: 'أمس', in_days: 'خلال',
    days: 'يوم', orders: 'الطلبات',
    cu_last_in: 'آخر زيارة', cu_never_bought: 'ما اشترى بعد', cu_quiet: 'انقطع',
    cu_quiet_only: 'المنقطعون', cu_owes_only: 'عليهم دين',
    cu_rhythm: 'بيجي', cu_every_n: 'كل {n} يوم', cu_quiet_after: 'بيُعتبر منقطع بعد {n} يوم',
    cu_rhythm_unknown: 'بدّه ثلاث مشتريات حتى نعرف',
    cu_none_size: 'ما في حدا مسجّل بيلبس {n}',
    cu_none_size_sub: 'القياسات بتطلع من المشتريات الفعلية، فالقياس الجديد بيبيّن بعد أول عملية بيع.',
    cu_clear_size: 'اعرض الكل',
    cu_none_search: 'ما في حدا بهالاسم أو الرقم',
    cu_none_search_sub: 'البحث بيلاقي الاسم بالعربي وباللاتيني، والرقم بأي صيغة. إذا فعلاً زبون جديد، ضيفه.',
    cu_none_debt: 'لا أحد مدين لك',
    cu_none_debt_sub: 'مبيعات الدين بتظهر هون لحتى تنسدّ.',
    cu_none_quiet: 'الكل ملتزم بعادته',
    cu_none_quiet_sub: 'الزبون بيطلع هون لما يتأخر عن المدة اللي اعتاد يرجع فيها.',
    cu_none_archived: 'ما في شي مؤرشف',
    cu_none_archived_sub: 'الزبون المؤرشف بيحتفظ بكل فواتيره — بس بينخفي عن القائمة.',
    cu_none_gold: 'ما في زبائن ذهبيين بعد',
    cu_none_gold_sub: 'النقاط بتزيد مع كل شرية، وأول الزبائن الدائمين رح يوصلوا لهون لحالهم.',
    cu_none_at_all: 'ما في زبائن بعد',
    cu_none_at_all_sub: 'ضيف أول واحد، وبيبلّش المحل يتذكّر مين بيشتري شو.',
    cu_gone: 'ما في زبون بهالرقم',
    cu_gone_sub: 'يمكن الرابط قديم، أو الحساب المسجّل دخول ما بيقدر يشوف هالشخص.',
    cu_timeline: 'كل شي بالترتيب', cu_events: 'حدث',
    cu_tl_empty: 'لسا ما صار شي',
    cu_tl_empty_sub: 'المشتريات والنقاط والتوصيلات كلها بتظهر هون وقت ما تصير.',
    cu_from_invoice: 'من الفاتورة {n}', cu_spent_on: 'مصروفة على {n}',
    cu_details: 'التفاصيل', cu_no_address: 'ما في عنوان مسجّل', cu_no_note: 'ما في ملاحظة',
    cu_no_sizes: 'ما اشترى شي بعد، فما في قياسات نبني عليها',
    cu_size_drift: 'كان ياخد {a} وصار ياخد {b} — يا تغيّر قياسه، يا عم يشتري لغيره.',
    cu_open_profile: 'افتح الملف الكامل',
    cu_edit: 'تعديل الزبون', cu_addr_ph: 'الشارع والبناء والطابق',
    cu_note_ph: 'أي شي بيستاهل يُتذكّر',
    cu_note_seen: 'الملاحظة بيشوفها أي حدا بيقدر يشوف الزبائن.',
    cu_archive_ask: 'أرشفة {n}؟',
    cu_archive_note: 'ما بينحذف شي. فواتيره بتبقى متل ما هي، وفلتر المؤرشف بيرجّعه.',
    address: 'العنوان', note: 'ملاحظة', edit: 'تعديل', dl_one: 'توصيلة', by: 'بواسطة',
    messages: 'الرسائل', currency: 'العملة',
    if_sold_all: 'لو بيع كل شيء', bulk_title: 'إجراء جماعي', undo: 'تراجع',
    warehouse: 'المستودع', print_done: 'تمّ', yi_title: 'فاتورة الشريك',
    cap_window: 'المعروض آخر {a} من أصل {b} {n} — كل رقم بهالشاشة محسوب على هالـ{a}، مو على المحل كله.',
    cap_of: 'من أصل {b}', rows: 'سطر',
    nt_showing: 'المعروض أحدث {a} من أصل {b} رسالة.',
    cu_tl_more: 'اعرض {n} حدث أقدم',
    cu_card: 'كرت الولاء', cu_card_already: '{n} مربوط أصلاً بهالبيعة',
    cu_card_swapped: 'انبدّل',
    hw_roll_claim: 'القياس يلي البرنامج بيفترضه للملصق',
    hw_roll_note: 'علامات الزوايا الأربع بتوقع على حواف الملصق يلي البرنامج متوقعه. إذا وقعت على زوايا الملصق الحقيقي، القياس فوق صحيح. إذا وقعت جواته أو طلعت برّاته، هالقياس هو يلي لازم يتغيّر — قيس الملصق وخبّرنا كم ملم.',
    cu_take_payment: 'قبض دفعة', cu_owed: 'الباقي',
    cu_then_now: 'وقتها / هلق', cu_then: 'وقتها', cu_now: 'هلق',
    cu_then_now_note: 'أرقام الدولار تقريبية — عمود «وقتها» بسعر الصرف المثبّت على كل فاتورة، وعمود «هلق» بسعر اليوم. ولا واحد منهن هو المبلغ المستحق؛ المستحق هو الرقم بالعمود الأول.',
    cu_part_paid: 'مدفوع منها {n}', cu_debt_settled: 'كل شي مسدّد.',
    cu_pay_note: 'هاد مصاري داخلة عالدرج، بهالوردية. ما بينلغى بإعادته، فالمبلغ بينتحقّق مقابل يلي فعلاً باقي.',
    cu_pay_amount_needed: 'اكتب المبلغ', cu_debt_cleared: 'انسدّ كامل',
    cu_still_owed: 'باقي {n}',
    cu_credit_limit: 'سقف الدين ($)', cu_no_limit: 'بدون سقف',
    cu_no_credit: 'الدين', cu_credit_ok: 'مسموح', cu_credit_none: 'ممنوع الدين',
    cu_credit_note: 'السقف بالدولار، فبيضل يعني نفس الشي بعد سنة. اتركه فاضي يعني بدون سقف — و٠ يعني ممنوع الدين نهائياً. التجاوز بينبّه؛ خيار «ممنوع الدين» بيرفض.',
    cu_limit_of: 'السقف {n}',
    cu_over_limit: 'تجاوز سقف الدين',
    cu_over_limit_msg: '{n} صار عليه تقريباً {a}، والسقف {b}. البيعة تمّت.',
    cu_merge: 'دمج المكرّر', cu_merge_pick: 'هاد',
    cu_merge_ask: 'أي سجل هو نفس الشخص {n}؟',
    cu_merge_which: 'السجل يلي بدك تدمجه',
    cu_merge_note: 'السجل يلي إنت عليه هلق هو يلي بيبقى. التاني بينأرشف مع إشارة لهون — ما بينحذف شي، وكل فاتورة بتحتفظ بالاسم يلي انكتب فيها.',
    cu_merge_no_obvious: 'ما في مكرّر واضح',
    cu_merge_no_obvious_sub: 'ما في حدا تاني بنفس الاسم أو الرقم. دوّر لتختار واحد بأي حال.',
    cu_merge_confirm: 'دمج {a} ضمن {b}؟',
    cu_merge_sales: 'مشترياته الـ{n} بتنتقل، ومعها النقاط يلي جابتها.',
    cu_merge_points: 'النقاط بتنجمع: {a} + {b} = {c}.',
    cu_merge_archived: '{n} بينأرشف، ما بينحذف.',
    cu_merge_irreversible: 'ما في زر تراجع لهالعملية.',
    cu_merged: 'انتقلت {n} عملية شراء',
    cu_merged_into: 'هالسجل انندمج ضمن {n}.',
    pj_link: 'ربط زبون', pj_link_ask: 'لمين الشغلة {n}؟',
    pj_link_note: 'اربط بس إذا إنت متأكد. الشغلة يلي بتنربط بالاسم بين كتابتين بتتعلّق بالشخص الغلط، وعشان هيك ما انحزر عنك شي.',
    wa_none_sub: 'لما ينسأل عن قياس عالكاشير وما يكون موجود، بينتسجّل هون لحاله مين طلبه.',
    wa_note: 'ما حدا كتب شي من هاد — كل سطر انسجّل لما حدا طلب هالقياس عالكاشير وما كان موجود. السطر بيصير أخضر لما يرجع القياس.',
    wa_told: 'انسجّل إنه انخبر',
    sa_attach: 'ربط زبون', sa_pick: 'اربط',
    sa_ask: 'لمين كانت {n}؟',
    sa_note: 'النقاط اللي كانت الفاتورة تجيبها بتنضاف هلق، على السعر يلي انباعت فيه. الاسم المطبوع على الفاتورة ما بيتغيّر — هو سجل لتلك اللحظة.',
    wa_wanted: 'طلب {n}', wa_answered: 'رجعنا خبّرناه',
    wa_still_waiting: 'لسا مستني', wa_wants: 'طلبه وما كان موجود',
    wa_none: 'ما في شي معلّق',
    wa_tell: 'تم إبلاغه',
    pr_wear: 'بيلبسوه', pr_wearers_hint: 'الزبائن المسجّلين يلي بياخدوا هالقياس — افتح القائمة',
    dl_failed_before: '{n} توصيلة سابقة ما وصلت',
    ly_card: 'كرت الأختام', ly_stamps: 'ختم', ly_full: 'الكرت كامل — في هدية مستحقة',
    ly_to_go: 'باقي {n} ختم', ly_full_only: 'كرت كامل',
    ly_owed_many: 'كمّل {n} كروت وما استلم عن ولا واحد لهلق.',
    ly_past: 'استُبدل سابقاً', ly_used_n: '{n} من {r} ختم',
    ly_redeem: 'استبدال الكرت', ly_redeemed: 'انستبدل الكرت · {n} ختم',
    ly_carried: 'وبقي {n} للكرت الجديد', ly_no_note: 'ما انسجّل شي',
    ly_redeem_ask: 'استبدال كرت كامل لـ {n}؟',
    ly_given: 'شو أخد؟', ly_given_ph: 'جزمة مجاناً، حسم ٢٠٪، أولوية على الجديد…',
    ly_redeem_note: 'ما في شي بينحسب لحاله — إنت بتقرّر شو بيساوي الكرت، وهون بينسجّل شو انعطى فعلاً. الأختام الزيادة بتنتقل للكرت الجديد.',
    ly_mode: 'شو بيشتغل المحل', ly_mode_points: 'نقاط بس', ly_mode_stamps: 'كروت أختام بس',
    ly_mode_both: 'نقاط وكروت أختام', ly_mode_off: 'ولا واحد',
    ly_block: 'النقاط المستبدلة دفعة وحدة',
    ly_required: 'الأختام لتكميل الكرت', ly_per: 'الختم بينحسب لكل',
    ly_per_item: 'قطعة مشتراة', ly_per_visit: 'زيارة',
    ly_stamps_note: 'الأختام بتنحسب من المشتريات الفعلية، فإلغاء أي فاتورة بيرجّع أختامها لحاله. ما في شي مخزّن، وما في شي بينحرف.',
    cu_visits: 'زيارات', cu_debt: 'عليه', cu_since: 'زبون منذ',
    cu_voided: 'ملغاة',
    cu_sort: 'ترتيب', cu_sort_recent: 'الأحدث أولاً', cu_sort_name: 'الاسم',
    cu_sort_spend: 'الإنفاق', cu_sort_visits: 'الزيارات', cu_sort_debt: 'الدين',
    cu_showing: 'معروض {a} من {b} — ضيّق البحث لتشوف الباقي',
    cu_phone_taken: 'انحفظ — بس الرقم مسجّل باسم {n}',
    cu_phone_taken_archived: 'انحفظ — بس الرقم مسجّل باسم {n} (زبون مؤرشف)',
    cu_view: 'افتح',
    cu_atrisk_title: 'نافذة الخطر',
    cu_atrisk_days: 'كم يوم بلا شراء حتى يُعتبر الزبون معرّضاً للفقدان',
    cu_atrisk_note: 'يغيّر الشارة الحمراء وفلتر المعرّضين وكل العدّادات، لكل الحسابات. ينحفظ على الخادم.',
    fam_Footwear: 'أحذية', fam_Jeans: 'جينز', fam_Tops: 'ملابس',

    print_title: 'أعمال الطباعة', print_sub: 'كل طلب تيشيرت من التصميم حتى التسليم',
    jobs_month: 'طلبات هذا الشهر', on_time: 'التسليم بالوقت', print_revenue: 'إيراد الطباعة',
    paid_partner: 'المدفوع ليلا وير',
    partner_access: 'وصول شريك',
    partner_note: 'هذا كل ما يراه شريك الطباعة. لا تكاليف، لا أرقام هواتف زبائن، لا مخزون، ولا أسعارك.',
    overdue: 'متأخر', design_note: 'ملاحظة التصميم', drag_hint: 'اسحب البطاقة بين الأعمدة لتغيير المرحلة',

    reports_title: 'التقارير', reports_sub: 'الأرقام الحقيقية خلف المحل',
    tab_sales: 'المبيعات', tab_profit: 'الأرباح', tab_inventory: 'قيمة المخزون',
    tab_employees: 'الموظفون', tab_suppliers: 'الموردون',
    export_excel: 'تصدير Excel', export_pdf: 'تصدير PDF', revenue: 'الإيراد',
    profit: 'الربح', units: 'القطع', capital_in_stock: 'رأس المال المجمّد بالمخزون',
    retail_value: 'قيمة البيع', role: 'الوظيفة', salary: 'الراتب الشهري',
    next_payment: 'الدفعة القادمة', outstanding: 'المستحق', due: 'الاستحقاق',
    supplier: 'المورّد', category: 'الفئة', sales_made: 'المبيعات المنجزة',
    avg_basket: 'متوسط الفاتورة', invoices: 'الفواتير', best_sellers: 'الأكثر مبيعاً',
    generating: 'جاري التجهيز…', export_ready: 'الملف جاهز',

    whatsapp: 'رقم الواتساب', 
    back: 'رجوع',
    pending: 'بانتظار التأكيد',
    confirmed: 'مؤكد', 

    settings_title: 'الإعدادات', settings_sub: 'القواعد والصلاحيات والمال',
    roles_perms: 'الصلاحيات والأدوار', exchange_rate: 'سعر الصرف',
    loyalty_rules: 'قواعد الولاء', reminders: 'التذكيرات التلقائية', branding: 'الهوية',
    save_changes: 'حفظ التعديلات', role_admin: 'مدير عام', role_manager: 'مدير',
    role_cashier: 'كاشير', role_warehouse: 'مستودع', permission: 'الصلاحية',
    points_per: 'نقاط لكل ١٠٠٠ ل.س', point_value: 'قيمة النقطة الواحدة',
    rate_hint: '١ دولار يساوي', shop_name: 'اسم المحل', accent_colour: 'اللون المميز',

    next: 'التالي', 
    of: 'من', close: 'إغلاق', cancel: 'إلغاء', save: 'حفظ',
    all_word: 'الكل', none: 'لا شيء', yes: 'نعم', no: 'لا', remove: 'حذف',
    print_job: 'طلب طباعة',
    size_gap: 'فجوة قياس', gap_only: 'فجوات القياسات', pieces: 'قطعة', all_new: 'كلها جديدة',

    /* شاشة التحميل (js/splash.js) */
    sp_loading: 'جارِ تحميل المحل', sp_ready: 'جاهز',
    sp_m_config: 'إعدادات المحل', sp_m_catalogue: 'الكتالوج', sp_m_customers: 'العملاء',
    sp_m_sales: 'المبيعات', sp_m_movements: 'المخزون', sp_m_partner: 'طلبات الطباعة',
    sp_m_purchase: 'طلبات الشراء', sp_m_money: 'الصندوق', sp_m_counts: 'الجرد',
    sp_m_suppliers: 'الموردون', sp_m_employees: 'الموظفون', sp_m_alerts: 'التنبيهات',
    sp_m_dashboard: 'لوحة المتابعة', sp_m_reports: 'التقارير', sp_m_labelTemplates: 'الملصقات',
    sp_m_mirror: 'النسخة السحابية',
    sp_u_warehouse: 'مستودع', sp_u_warehouse_1: 'مستودع',
    sp_u_product: 'منتج', sp_u_product_1: 'منتج',
    sp_u_customer: 'عميل', sp_u_customer_1: 'عميل',
    sp_u_invoice: 'فاتورة', sp_u_invoice_1: 'فاتورة',
    sp_u_movement: 'حركة مخزون', sp_u_movement_1: 'حركة مخزون',
    sp_u_job: 'طلب', sp_u_job_1: 'طلب',
    sp_u_order: 'طلب شراء', sp_u_order_1: 'طلب شراء',
    sp_u_count: 'جرد', sp_u_count_1: 'جرد',
    sp_u_supplier: 'مورد', sp_u_supplier_1: 'مورد',
    sp_u_employee: 'موظف', sp_u_employee_1: 'موظف',
    sp_u_alert: 'تنبيه', sp_u_alert_1: 'تنبيه',
    sp_u_template: 'قالب', sp_u_template_1: 'قالب',
    sp_u_row_waiting: 'صف بانتظار الرفع', sp_u_row_waiting_1: 'صف بانتظار الرفع',
    sp_u_row_pulled: 'صف مسحوب من السحابة', sp_u_row_pulled_1: 'صف مسحوب من السحابة',
    sp_shift_open: 'الوردية مفتوحة', sp_shift_closed: 'لا وردية مفتوحة',
    sp_computed: 'محسوبة', sp_mirror_ok: 'متطابقة',
    sp_mirror_off: 'غير مهيأة', sp_mirror_attention: 'تحتاج انتباهاً'
  }
};

function t(key) {
  var d = I18N[OG.lang] || I18N.en;
  return (d[key] !== undefined ? d[key] : (I18N.en[key] !== undefined ? I18N.en[key] : key));
}
