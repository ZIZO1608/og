...
var Bulk = (function () {
  ...
  function run(sc, a) {
    ...
    if (sc === 'jobs') {
      var jl = selJobs();
      if (a === 'export') { exportSelection(sc); return; }
      var prev = jl.map(function (j) { return { j: j, stage: j.stage, hist: j.history.slice() }; });
      ...
      stageUndo(n + ' ' + t('bk_moved'), function () {
        prev.forEach(function (p) { p.j.stage = p.stage; p.j.history = p.hist; });
      });
