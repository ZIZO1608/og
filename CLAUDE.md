...
| Shape | Tables | How |
|---|---|---|
| **Cursor** | `products` `variants` `stock` `customers` `sales` (+`sale_items`) `deliveries` | Replays `change_log` from a cursor held in Supabase `sync_state` |
| **Mirror** | `config` `role_permissions` `label_templates` | Pushed whole every run, **and rows deleted here are deleted there** |
| **Append-only** | `fx_rates` `stock_movements` | Pushed above the highest `id` already sent |
...
- **The partner half is still invented.** Print jobs, partner invoices, suppliers, staff, clubs and the
  job/WhatsApp message threads have no tables, no endpoints and no mirror — they are generated by
  `js/data.js` on every load. `Shop.load()` replaces only config, rate, warehouses, products,
  customers, sales and movements; everything else on screen is seed data. Nothing a partner does in the
  Yalla Wear portal is recorded anywhere.
